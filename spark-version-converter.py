...
files = [
    "sql/src/main/scala/org/apache/sedona/sql/UDF/UdfRegistrator.scala",
    ...
    if argv[1] == "spark2":
        with fileinput.FileInput(filepath, inplace=True) as file:
            for line in file:
                if spark2_anchor in line:
                    conversion_result_spark2 = (
                        spark2_anchor + " " + enable_version(line)
                    )
                elif spark3_anchor in line:
                    conversion_result_spark3 = (
                        spark3_anchor + " " + disable_version(line)
                    )
                else:
                    print(line, end="")
        ...
        with fileinput.FileInput(filepath, inplace=True) as file:
            for line in file:
                if spark2_anchor in line:
                    conversion_result_spark2 = (
                        spark2_anchor + " " + disable_version(line)
                    )
                elif spark3_anchor in line:
                    conversion_result_spark3 = (
                        spark3_anchor + " " + enable_version(line)
                    )
                else:
                    print(line, end="")
            return conversion_result_spark2 + " and " + conversion_result_spark3
