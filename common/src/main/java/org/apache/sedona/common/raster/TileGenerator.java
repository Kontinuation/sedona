...
public class TileGenerator {
  ...
  /**
   * Generate tiles from an out-db grid coverage. The generated tiles are also out-db grid
   * coverages. The generated tiles will have various geo-referencing and band indices metadata,
   * while sharing the same out-db raster file with the original grid coverage. No pixel data will
   * be copied during this process.
   *
   * <p>Please note that tiling for out-db grid coverage does not support padding
   *
  ...
  public abstract static class TileIterator implements Iterator<Tile> {
    ...
    @Override
    public Tile next() {
      // Check if current tile coordinate id valid
  ...
  public static class OutDbTileIterator extends TileIterator {
    private final int[] bandIndices;
    private final int tileWidth;
    private final int tileHeight;
    private final AffineTransform2D affine;
    ...
    private final int[] tileBandIndices;
    private final int imageWidth;
    private final int imageHeight;
    ...
    public OutDbTileIterator(
      ...
      outDbPath = gridCoverage2D.getOutDbPath();
      serializedConf = gridCoverage2D.getSerializedConfiguration();
      outDbParams = gridCoverage2D.getOutDbParams();
      tileBandIndices = new int[bandIndices.length];
      for (int i = 0; i < bandIndices.length; i++) {
        int bandIndex = bandIndices[i] - 1;
        tileBandIndices[i] = outDbBandIndices[bandIndex];
      }
      imageWidth = gridRange.width;
      imageHeight = gridRange.height;
      int numTileX = (int) Math.ceil((double) imageWidth / tileWidth);
      int numTileY = (int) Math.ceil((double) imageHeight / tileHeight);
      initialize(numTileX, numTileY);
    }
    ...
    protected Tile generateTile() {
      ...
      // For out-db rasters, we only need to change the geo-referencing information and select a
      // subset of bands. The reference to data (path and conf) does not need to be changed.
      GridSampleDimension[] sampleDimensions = new GridSampleDimension[bandIndices.length];
      for (int k = 0; k < bandIndices.length; k++) {
        int bandIndex = bandIndices[k] - 1;
        sampleDimensions[k] = gridCoverage2D.getSampleDimension(bandIndex);
      }
      OutDbGridCoverage2D tile =
          OutDbGridCoverage2D.create(
              gridCoverage2D.getName(),
              gridGeometry2D,
              sampleDimensions,
              tileBandIndices,
              outDbPath,
