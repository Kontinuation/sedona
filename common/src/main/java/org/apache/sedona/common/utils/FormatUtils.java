...
public class FormatUtils<T extends Geometry> implements Serializable {
    ...
    public FormatUtils(
        ...
        // Only the following formats are allowed to use this format mapper because each input has
    ...
    public static List<String> readGeoJsonPropertyNames(String geoJson) {
        if (geoJson == null) {
            logger.warn(
                    "The given GeoJson record is null and cannot be used to find property names");
            return null;
        }
    ...
    private void handleNonSpatialDataToGeometry(
            Geometry geometry, List<String> splitedGeometryData) {
    ...
    public Geometry readWkb(String line) throws ParseException {
        ...
        // For some unknown reasons, the wkb reader cannot be used in transient variable like the
        // wkt reader.
    ...
    public Coordinate[] readCoordinates(String line) {
        ...
        for (int i = this.startOffset; i <= actualEndOffset; i += 2) {
            coordinates[(i - startOffset) / 2] =
                    new Coordinate(
                            Double.parseDouble(columns[i]), Double.parseDouble(columns[i + 1]));
        }
        if (carryInputData) {
                ...
                default:
                    {
                        if (this.geometryType == null) {
                            throw new IllegalArgumentException(
                                    "[Sedona][FormatMapper] You must specify GeometryType when you"
                                            + " use delimiter rather than WKB, WKT or GeoJSON");
                        } else {
    ...
    private Geometry createGeometry(Coordinate[] coordinates, GeometryType geometryType) {
        ...
        switch (geometryType) {
            ...
            case LINESTRING:
                ...
                break;
            case RECTANGLE:
                // The rectangle mapper reads two coordinates from the input line. The two
                // coordinates are the two on the diagonal.
                ...
                break;
                // Read string to point if no geometry type specified but Sedona should never reach
                // here
            default:
                geometry = geometryFactory.createPoint(coordinates[0]);
        }
