...
/**
 * Subdivides a polygon into boxes. The boxes is an approximated coverage of the polygon. This allows us
 * to get rid of indexing a big concave or slim polygon using its extent, where the extent is much larger
 * than the actual polygon. This is also useful for indexing a polygon with large holes.
 */
public class SubdividedPolygonUsingBoxes implements Iterator<Geometry> {
    ...
    public static List<Envelope> splitToBoxes(PreparedGeometry preparedGeometry, SubdivideOptions options) {
    ...
    private static void splitToBoxes(PreparedGeometry preparedGeometry,
                                     SubdivideOptions options, Envelope extent,
                                     int depth, List<Envelope> output) {
