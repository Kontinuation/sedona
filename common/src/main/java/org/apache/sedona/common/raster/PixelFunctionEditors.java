...
public class PixelFunctionEditors {
    ...
    /**
     * Returns a raster by replacing the values of pixels in a specified geometry region. It
     * converts the Geometry to a raster using RS_AsRaster.
     *
     * @param raster Raster to be edited
     * @param band Band of the raster to be edited
    ...
    public static GridCoverage2D setValues(
        ...
        if (keepNoData) {
            noDataValue = RasterBandAccessors.getBandNoDataValue(raster, band);
            rasterizedGeom =
                    RasterConstructors.asRaster(geom, raster, bandDataType, value, noDataValue);
        } else {
        ...
        // Taking a shortcut if the provided geometry is a Point or MultiPoint, then taking the lat
        // lon
            ...
            for (Coordinate pointCoordinate : coordinates) {
                int[] pointLocation =
                        raster.getGridGeometry()
                                .worldToGrid(
                                        new DirectPosition2D(pointCoordinate.x, pointCoordinate.y))
                                .getCoordinateValues();
                double[] pixel =
                        rasterCopied.getPixel(pointLocation[0], pointLocation[1], (double[]) null);
        ...
        // Converting geometry to raster and then iterating through them
            ...
            int widthRegion =
                    Math.min(widthGeometryRaster - geometryX, widthOriginalRaster - rasterX);
            int heightRegion =
                    Math.min(heightGeometryRaster - geometryY, heightOriginalRaster - rasterY);

            for (int j = 0; j < heightRegion; j++) {
                for (int i = 0; i < widthRegion; i++) {
                    double[] pixel =
                            rasterCopied.getPixel(rasterX + i, rasterY + j, (double[]) null);
                    // [0] as only one band in the rasterized Geometry
                    double pixelNew =
                            rasterizedGeomData
                                    .getPixel(geometryX + i, geometryY + j, (double[]) null)[0];
                    // skipping 0 from the rasterized geometry as
                    if (pixelNew == 0
                            || keepNoData
                                    && noDataValue != null
                                    && noDataValue == pixel[band - 1]) {
                        continue;
                    } else {
    ...
    /**
     * Returns a raster by replacing the values of pixels in a specified geometry region. It
     * converts the Geometry to a raster using RS_AsRaster. A convenience function with keepNoData
     * as false.
     *
