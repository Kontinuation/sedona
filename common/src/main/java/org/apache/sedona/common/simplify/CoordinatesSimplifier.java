...
public class CoordinatesSimplifier {
    public static Coordinate[] simplifyInPlace(
            Coordinate[] geom, double tolerance, int minPointsExternal) {
        if (geom.length < 3 || geom.length <= minPointsExternal) {
            ...
            while (!iteratorStack.isEmpty()) {
                CoordinateSplitter.SplitInPlace splitInPlaceRes =
                        CoordinateSplitter.splitInPlace(geom, itFirst, itLast, itTool);
