...
/**
 * Lazy-loaded OutDbGridCoverage2D is a wrapper for OutDbGridCoverage2D that only reads the referenced raster data when
 * necessary, even when we only need to read the metadata of the raster, such as its width, height and geo-referencing
 * information. This is for avoiding unnecessary data loading when we only want to wrap a URI to a raster, without any
 * other overhead.
 */
public class LazyLoadOutDbGridCoverage2D extends OutDbGridCoverage2D {
    ...
    public static LazyLoadOutDbGridCoverage2D readKryo(UnsafeInput in, byte[] serializedConf) {
        ...
        if (serializedConfLength > 0) {
            ...
            // serialized configuration is not present, user must provide a configuration to reconstruct the raster
