...
/**
 * This is a more comprehensive class that is capable of subdividing geometries based on their extents.
 * It can handle various kinds of geometries, including points, lines, and polygons.
 */
public class ExtentBasedGeometrySubDivider {
    ...
    /**
     * Subdivide a geometry into smaller geometries based on the maximum number of vertices, width, height, and area.
    ...
    private Iterator<Geometry> subdivideMultiPoint(MultiPoint geometry) {
        switch (options.multiPointSubDivider) {
            ...
            default:
                throw new IllegalArgumentException("Unsupported multi-point sub-divider: " +
                        options.multiPointSubDivider);
        }
    }
    ...
    private Iterator<Geometry> subdivideLineString(LineString geometry) {
        switch (options.lineStringSubDivider) {
            ...
            default:
                throw new IllegalArgumentException("Unsupported line-string sub-divider: " +
                        options.lineStringSubDivider);
        }
    }
    ...
    private Iterator<Geometry> subdividePolygon(Polygon geometry) {
        switch (options.polygonSubDivider) {
            ...
            default:
                throw new IllegalArgumentException("Unsupported polygon sub-divider: " +
                        options.polygonSubDivider);
        }
    }
}
