...
public class RasterConstructors {
     ...
     * @param useGeometryExtent The way to generate extent of the resultant raster. Use the extent
     *     of the geometry to convert if true, else use the extent of the reference raster
    ...
    private static List<Object> rasterization(
        ...
        if (metadata[4] < 0) {
            throw new IllegalArgumentException(
                    String.format(
                            "ScaleX %f of the raster is negative, it should be positive",
                            metadata[4]));
        }
        // ScaleY > 0
        if (metadata[5] > 0) {
        ...
        if (useGeometryExtent) {
            bound =
                    JTS.getEnvelope2D(
                            geom.getEnvelopeInternal(), raster.getCoordinateReferenceSystem2D());
        } else {
        ...
        if (width == 0 && height == 0) {
            bound =
                    new Envelope2D(
                            ...
                            bound.getCenterX() - scaleX * 0.5,
                            bound.getCenterY() - scaleY * 0.5,
                            scaleX,
                            scaleY);
            width = 1;
            height = 1;
     ...
     * the given number of empty bands. The bounding envelope is defined by the upper left corner
     * and the scale. The math formula of the envelope is: minX = upperLeftX = lowerLeftX, minY
     ...
     * @param widthInPixel the width of the raster, in pixel
     * @param heightInPixel the height of the raster, in pixel
     * @param upperLeftX the upper left corner of the raster, in the CRS unit. Note that: the minX
     *     of the envelope is equal to the upperLeftX
     * @param upperLeftY the upper left corner of the raster, in the CRS unit. Note that: the minY
     *     of the envelope is equal to the upperLeftY + height * scaleY
    ...
    public static GridCoverage2D makeEmptyRaster(
        ...
        WritableRaster raster =
                RasterFactory.createBandedRaster(
                        RasterUtils.getDataTypeCode(bandDataType),
                        widthInPixel,
                        heightInPixel,
                        numBand,
                        null);
    ...
    public static GridCoverage2D makeNonEmptyRaster(
        ...
        WritableRaster raster =
                RasterFactory.createBandedRaster(
                        ...
                        heightInPixel,
                        numBand,
                        null); // create a raster with double values
     ...
     * number of bands of the reference raster is determined by the size of values. The size of
     * values should be multiple of width * height of the reference raster.
     *
    ...
    public static GridCoverage2D makeNonEmptyRaster(
        ...
        if (values.length % valuesPerBand != 0) {
            throw new IllegalArgumentException(
                    "The size of values should be multiple of width * height of the reference"
                            + " raster");
        }
        ...
        for (int i = 0; i < numBands; i++) {
            double[] bandValues =
                    Arrays.copyOfRange(values, i * valuesPerBand, (i + 1) * valuesPerBand);
     ...
     * @param padNoDataValue the no data value for padded tiles, only used when padWithNoData is
     *     true. If the value is NaN, the no data value of the original band will be used.
     * @return the tiles
     */
        ...
        if (bandIndices == null || bandIndices.length == 0) {
            ...
            for (int bandIndex : bandIndices) {
                if (bandIndex <= 0 || bandIndex > numBands) {
                    throw new IllegalArgumentException(
                            String.format(
                                    "Provided band index %d is not present in the raster",
                                    bandIndex));
                }
            }
        }
    ...
    /**
     * Generate tiles from an in-db grid coverage. The generated tiles are also in-db grid
     * coverages. Pixel data will be copied into the tiles.
     *
    ...
    private static Tile[] doGenerateTiles(
        ...
        for (int i = 0; i < bandIndices.length; i++) {
            noDataValues[i] =
                    RasterUtils.getNoDataValue(
                            gridCoverage2D.getSampleDimension(bandIndices[i] - 1));
        }
        ...
        for (int tileY = 0; tileY < numTileY; tileY++) {
            for (int tileX = 0; tileX < numTileX; tileX++) {
                ...
                boolean needPadding =
                        padWithNoData && (rectWidth < tileWidth || rectHeight < tileHeight);
                ...
                GridSampleDimension[] sampleDimensions =
                        new GridSampleDimension[bandIndices.length];
                ...
                for (int k = 0; k < bandIndices.length; k++) {
                    ...
                    // Copy sample dimensions from source bands, and pad with no data value if
                    // necessary
                    ...
                    if (needPadding && !Double.isNaN(padNoDataValue)) {
                        sampleDimension =
                                RasterUtils.createSampleDimensionWithNoDataValue(
                                        sampleDimension, padNoDataValue);
                        noDataValue = padNoDataValue;
                    }
                    sampleDimensions[k] = sampleDimension;

                    // Copy data from original image to tile image
                    ImageUtils.copyRasterWithPadding(
                            sourceRaster, bandIndex, raster, k, noDataValue);
                }
    ...
    public static GridCoverage2D[] rsTile(
            ...
            boolean padWithNoData) {
        return rsTile(
                gridCoverage2D, bandIndices, tileWidth, tileHeight, padWithNoData, Double.NaN);
    }
