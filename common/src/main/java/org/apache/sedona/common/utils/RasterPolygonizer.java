...
/**
 * This raster polygonizer code is mostly taken from GDALPolygonizeT in GDAL library 3.8.5, which
 * implements a two-arm algorithm for raster polygonization.
 */
public class RasterPolygonizer {
  ...
  public static List<PolygonWithValue> polygonize(
    ...
    RasterPolygonizer.PolygonWriter polygonWriter =
        new RasterPolygonizer.PolygonWriter(geoTransform);
    RasterPolygonizer.Polygonizer polygonizer =
        new RasterPolygonizer.Polygonizer(-1, polygonWriter);
    ...
    for (int iY = 0; iY < nYSize + 1; iY++) {
      ...
      if (iY < nYSize) {
        for (int iX = 0; iX < nXSize; iX++) {
          // TODO: maybe we can reserve -1 as the lookup result for -1 polygon id in the
          // panPolyIdMap,
          //       so the this expression becomes: panLastLineId[iX] = *(oFirstEnum.panPolyIdMap +
          // panThisLineId[iX]).
  ...
  static void ProcessArmConnections(TwoArm current, TwoArm above, TwoArm left) {
    ...
    int nArmConnectionType =
        ((above.solidVertical ? 1 : 0) << BIT_ABOVE)
    ...
    void addArcToRing(
      ...
      do {
        ...
        for (int i = 0; i < arc.points.size(); ++i) {
          ...
          double dfX =
              geoTransform[0] + oPixel.col * geoTransform[1] + oPixel.row * geoTransform[2];
          double dfY =
              geoTransform[3] + oPixel.col * geoTransform[4] + oPixel.row * geoTransform[5];
