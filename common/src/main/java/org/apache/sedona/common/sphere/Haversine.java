...
public class Haversine {
    ...
    public static Envelope expandEnvelope(Envelope envelope, double distance, double sphereRadius) {
        ...
        if (newMinY <= -90 || newMaxY >= 90) {
            // The expanded envelope covers the pole, so its longitude range should be expanded to
            // [-180, 180]
        ...
        if (newMinX <= -180 || newMaxX >= 180) {
            // The expanded envelope crosses the anti-meridian. Expanding its longitude range to
            // [-180, 180] is
            // the best thing we can do, since we treat lon-lat as planar coordinates when running
            // spatial join.
