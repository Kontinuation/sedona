...
public class RasterBandEditors {
        ...
        if (replace) {
            if (rasterNoData == null) {
                throw new IllegalArgumentException(
                        "The raster provided doesn't have a no-data value. Please provide a raster"
                                + " that has a no-data value to use `replace` option.");
            }
    ...
    public static GridCoverage2D addBand(
        ...
        int width = RasterAccessors.getWidth(toRaster),
                height = RasterAccessors.getHeight(toRaster);
        ...
        if (RasterUtils.isDataTypeIntegral(dataTypeCode)) {
            int[] bandValues =
                    rasterData.getSamples(0, 0, width, height, fromBand - 1, (int[]) null);
    ...
    public static GridCoverage2D rasterUnion(
        ...
        return rasterUnion(
                rasterUnion(raster1, raster2, raster3, raster4, raster5, raster6), raster7);
    }
    ...
    public static GridCoverage2D clip(
        ...
        if (!crop) {
            ...
            if (isDataTypeIntegral) {
                ...
                double[] array =
                        ArrayUtils.toPrimitive(
                                Collections.nCopies(sizeOfArray, noDataValue)
                                        .toArray(new Double[sizeOfArray]));
                resultRaster.setSamples(0, 0, widthOriginalRaster, heightOriginalRaster, 0, array);
            }

            // rasterize the geometry to iterate over the clipped raster
            GridCoverage2D rasterized =
                    RasterConstructors.asRaster(geometry, raster, bandType, 150);
    ...
    /**
     * Return a clipped raster with the specified ROI by the geometry. No-data value will be taken
     * as the lowest possible value for the data type and crop will be `true`.
     *
     * @param raster Raster to clip
     * @param band Band number to perform clipping
     * @param geometry Specify ROI
