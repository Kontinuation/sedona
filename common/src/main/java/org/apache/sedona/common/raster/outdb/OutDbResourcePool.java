...
/**
 * A resource pool for reusing out-db raster resources. The byte ranges read by other
 * OutDbGridCoverage2D objects could be reused by newly created OutDbGridCoverage2D objects. What's
 * more, this also avoids repeated opening and closing of the same file, which is quite slow for
 * GeoTiff files. Please notice that this is a non-thread-safe class and should be used as a
 * thread-local variable.
 */
public class OutDbResourcePool {
  static final Logger logger = LoggerFactory.getLogger(OutDbResourcePool.class);
    ...
    OutDbResource(ResourceKey key, GridCoverage2D gridCoverage2D, ImageInputStream stream) {
      if (gridCoverage2D instanceof OutDbGridCoverage2D) {
        ...
        throw new IllegalArgumentException(
            "gridCoverage2D must not be an instance of OutDbGridCoverage2D");
      }
  ...
  public OutDbResourcePool(int freeResourcesCapacity) {
    ...
    logger.debug(
        "Creating OutDbResourcePool for thread {}, capacity: {}", threadId, freeResourcesCapacity);
  ...
  public OutDbResource acquire(ResourceKey key) {
    ...
    if (resource != null) {
      ...
      resource.refCount += 1;
      logger.debug(
          "Acquired OutDbResource object(ref={}, path={}) for thread {}. Pool stats: {}/{}",
          resource.refCount,
          key.path,
          threadId,
          freeResourceCount,
      ...
      logger.debug(
          "No OutDbResource object found for thread {}, path={}. Pool stats: {}/{}",
          threadId,
          key.path,
          freeResourceCount,
          allResources.size());
  ...
  public void add(OutDbResource resource) {
    ...
    if (existing == null) {
      // Add this resource to the pool, so that it can be shared by other out-db grid coverage
      // objects
      ...
      logger.debug(
          ...
          threadId,
          resource.resourceKey.path,
          freeResourceCount,
          allResources.size());
    } else {
      logger.debug(
          "Ignored adding duplicated OutDbResource object(ref={}, path={}) for thread {}. Pool stats: {}/{}",
          resource.refCount,
          resource.resourceKey.path,
          threadId,
          freeResourceCount,
  ...
  public void release(OutDbResource resource) {
    ...
    if (existing == null) {
      ...
      logger.debug(
          "Releasing new OutDbResource object for thread {}, path={}. Pool stats: {}/{}",
          threadId,
          resource.resourceKey.path,
          freeResourceCount,
          allResources.size());
    ...
    if (existing != resource) {
      resource.dispose();
      logger.debug(
          "Ignored releasing duplicated OutDbResource object(ref={}, path={}) for thread {}. Pool stats: {}/{}",
          resource.refCount,
          resource.resourceKey.path,
          threadId,
          freeResourceCount,
    ...
    logger.debug(
        "Released OutDbResource object(ref={}, path={}) for thread {}. Pool stats: {}/{}",
        resource.refCount,
        resource.resourceKey.path,
        threadId,
        freeResourceCount,
  ...
  public void verifyIntegrity() {
    ...
    if (count != freeResourceCount) {
      throw new IllegalStateException(
          "freeResourceCount does not match with the actual number of free resources");
    }
    count = 0;
    ...
    if (count != freeResourceCount) {
      throw new IllegalStateException(
          "freeResourceCount does not match with the length of freeResources list");
    }
  }
  ...
  protected void finalize() {
    logger.debug(
        "Finalizing OutDbResourcePool for thread {}: containing {}/{} objects",
        threadId,
        freeResourceCount,
        allResources.size());
  }
  ...
  private void drainReferenceQueue() {
    ...
    if (count > 0) {
      logger.debug(
          "Removed {} garbage collected OutDbResource objects for thread {}. Pool stats: {}/{}",
          count,
          threadId,
          freeResourceCount,
          allResources.size());
