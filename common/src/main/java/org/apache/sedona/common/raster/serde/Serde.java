...
public class Serde {
  ...
  public static byte[] serialize(GridCoverage2D raster, boolean withOutDbConfiguration)
      throws IOException {
    Kryo kryo = kryos.get();
    if (!(raster instanceof OutDbGridCoverage2D)) {
      ...
      // Get a serializable state of OutDbGridCoverage2D and serialize it. We can restore the
      // OutDbGridCoverage2D
      ...
      try (UnsafeOutput out = new UnsafeOutput(4096, -1)) {
        ...
        if (!isLazy) {
          OutDbGridCoverage2D.SerializableState state =
              outDbRaster.getSerializableState(withOutDbConfiguration);
          state.write(kryo, out);
        } else {
  ...
  public static GridCoverage2D deserialize(byte[] bytes, byte[] serializedConf)
      throws IOException, ClassNotFoundException {
    Kryo kryo = kryos.get();
