            ...
            case "BandedSampleModel":
                raster =
                        RasterFactory.createBandedRaster(
                                dataType, widthInPixel, heightInPixel, numBand, null);
                break;
            ...
            case "SinglePixelPackedSampleModel":
                {
                    if (dataType != DataBuffer.TYPE_INT) {
                        throw new IllegalArgumentException(
                                "only supports creating SinglePixelPackedSampleModel with int data"
                                        + " type");
                    }
                    if (numBand != 4) {
                        throw new IllegalArgumentException(
            ...
            case "MultiPixelPackedSampleModel":
                {
                    if (dataType != DataBuffer.TYPE_BYTE) {
                        throw new IllegalArgumentException(
                                "only supports creating MultiPixelPackedSampleModel with byte data"
                                        + " type");
                    }
                    if (numBand != 1) {
                        throw new IllegalArgumentException(
