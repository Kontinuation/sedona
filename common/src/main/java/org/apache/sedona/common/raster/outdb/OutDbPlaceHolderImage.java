...
/**
 * This image object works as a placeholder in OutDbGridCoverage2D. OutDbGridCoverage2D supports
 * loading the referenced raster image file in a deferred manner: it may not load the image file on
 * object construction, and user can happily retrieve geo-referencing information from the object.
 * When user retrieves pixel data from the object, or calls getRenderedImage(), OutDbGridCoverage2D
 * will load the actual raster image file.
 *
 * <p>Before the actual raster image is loaded, OutDbGridCoverage2D will use this placeholder object
 * to represent the image. This placeholder object contains the width, height, sample model and
 * color model of the image, so that the OutGridCoverage2D object will be successfully constructed
 * by calling the constructor of the parent (GridCoverage2D) class. The placeholder object will be
 * replaced by the actual raster image object when the actual raster image file is loaded.
 */
public class OutDbPlaceHolderImage implements RenderedImage {
  ...
  public OutDbPlaceHolderImage(int width, int height, int numBand, int dataType) {
    ...
    // It doesn't matter what sample model we are using here, as long as it gives us the correct
    // values for
