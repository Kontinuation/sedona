...
public class RasterPolygonEnumerator {
  ...
  private void mergePolygon(int srcId, int dstIdInit) {
    ...
    while (polyIdMap[dstIdFinal] != dstIdFinal) dstIdFinal = polyIdMap[dstIdFinal];
  ...
  private int newPolygon(int value) {
    ...
    if (nextPolygonId >= polyAlloc) {
      int polyAllocNew;
      if (polyAlloc < (Integer.MAX_VALUE - 20) / 2) polyAllocNew = polyAlloc * 2 + 20;
      else polyAllocNew = Integer.MAX_VALUE;
  ...
  public void completeMerges() {
    ...
    logger.debug(
        "Counted {} polygon fragments forming {} final polygons.", nextPolygonId, finalPolyCount);
  }
