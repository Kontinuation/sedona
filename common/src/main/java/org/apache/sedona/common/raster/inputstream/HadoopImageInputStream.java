...
public class HadoopImageInputStream extends ImageInputStreamImpl {
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    // relies on this method and assumes that partial reads only happens when reading EOF. This
    // might
    ...
    while (remaining > 0) {
      ...
      if (ret_len < 0) {
        ...
        if (len - remaining > 0) {
          ...
          // The partial read may happen when reading from S3 (S3AInputStream), and it may also
          // happen
