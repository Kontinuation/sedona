...
public class HadoopConfigSerializer {
  ...
  // serialized bytes.
  private static final LoadingCache<Configuration, byte[]> serializeCache =
      Caffeine.newBuilder().maximumSize(100).build(HadoopConfigSerializer::doSerialize);
  private static final LoadingCache<ByteBuffer, Configuration> deserializeCache =
      Caffeine.newBuilder().maximumSize(100).build(HadoopConfigSerializer::doDeserialize);
