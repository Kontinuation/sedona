...
public class RasterEditors {
            ...
            if (!Double.isNaN(RasterUtils.getNoDataValue(sampleDimensions[band]))) {
                sampleDimensions[band] =
                        RasterUtils.createSampleDimensionWithNoDataValue(
                                sampleDimensions[band],
                                castRasterDataType(
                                        RasterUtils.getNoDataValue(sampleDimensions[band]),
                                        newDataType));
            }
        }
    ...
    public static GridCoverage2D setGeoReference(
        ...
        String geoRedCoord =
                String.format(
                        "%f %f %f %f %f %f", scaleX, skewY, skewX, scaleY, upperLeftX, upperLeftY);
    ...
    public static GridCoverage2D resample(
        ...
        if (!useScale) {
            ...
            // height and width cannot have floating point, ceil them to next greatest integer in
            // that case.
            newWidth =
                    (int)
                            Math.ceil(
                                    Math.abs(envelope2D.getMaxX() - envelope2D.getMinX())
                                            / Math.abs(newScaleX));
            newHeight =
                    (int)
                            Math.ceil(
        ...
        if (!approximateEquals(upperLeftX, gridX) || !approximateEquals(upperLeftY, gridY)) {
            ...
            Point2D expectedGeoPoint =
                    RasterUtils.getWorldCornerCoordinates(
                            tempRaster,
                            expectedCellCoordinates[0] + 1,
                            expectedCellCoordinates[1] + 1);
            ...
            if (!approximateEquals(expectedGeoPoint.getX(), upperLeftX)) {
                if (!useScale) {
                    ...
                    // width cannot have floating point, ceil it to next greatest integer in that
                    // case.
                    newWidth =
                            (int)
                                    Math.ceil(
            ...
            if (!approximateEquals(expectedGeoPoint.getY(), upperLeftY)) {
                if (!useScale) {
                    ...
                    // height cannot have floating point, ceil it to next greatest integer in that
                    // case.
                    newHeight =
                            (int)
                                    Math.ceil(
        ...
        if ((!Objects.isNull(algorithm) && !algorithm.isEmpty())
                && (algorithm.equalsIgnoreCase("Bilinear")
                        || algorithm.equalsIgnoreCase("Bicubic"))) {
            ...
            newRaster =
                    (GridCoverage2D)
                            Operations.DEFAULT.resample(
                                    raster, null, gridGeometry, resamplingAlgorithm);
    ...
    public static GridCoverage2D normalizeAll(
            ...
            Double noDataValue) {
        return normalizeAll(
                rasterGeom, minLim, maxLim, normalizeAcrossBands, noDataValue, null, null);
    }
    ...
    public static GridCoverage2D normalizeAll(
        ...
        // Trigger safe mode if noDataValue is null - noDataValue is set to maxLim and data values
        // are normalized to range [minLim, maxLim-1].
        ...
        for (int bandIndex = 0; bandIndex < numBands; bandIndex++) {
            ...
            double currentMin =
                    normalizeAcrossBands
                            ? globalMin
                            : (minValue != null ? minValue : minValues[bandIndex]);
            double currentMax =
                    normalizeAcrossBands
                            ? globalMax
                            : (maxValue != null ? maxValue : maxValues[bandIndex]);
                ...
                for (int i = 0; i < bandValues.length; i++) {
                    if (bandValues[i] != bandNoDataValue) {
                        double normalizedValue =
                                minLim
                                        + ((bandValues[i] - currentMin)
                                                        * (maxLim - safetyTrigger - minLim))
            ...
            rasterGeom =
                    RasterBandEditors.setBandNoDataValue(rasterGeom, bandIndex + 1, noDataValue);
        }
    ...
    public static GridCoverage2D interpolate(
        ...
        WritableRaster raster =
                rasterData.createCompatibleWritableRaster(
                        RasterAccessors.getWidth(inputRaster),
                        RasterAccessors.getHeight(inputRaster));
        ...
        for (int bandIndex = 0; bandIndex < numBands; bandIndex++) {
            if (band == null || bandIndex == band - 1) {
                ...
                Double noDataValue =
                        RasterUtils.getNoDataValue(inputRaster.getSampleDimension(bandIndex));
                ...
                for (int y = 0; y < height; y++) {
                    for (int x = 0; x < width; x++) {
                        ...
                        if (Double.isNaN(value) || value == noDataValue) {
                            ...
                            interpolatedValue =
                                    (Double.isNaN(interpolatedValue))
                                            ? noDataValue
                                            : interpolatedValue;
                ...
                if (countNoDataValues == 0) {
                    gridSampleDimensions[bandIndex] =
                            RasterUtils.removeNoDataValue(
                                    inputRaster.getSampleDimension(bandIndex));
                }
            } else {
                raster.setSamples(
                        0,
