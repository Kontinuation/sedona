 ...
 * The difference between this class and {@link FileCacheImageInputStream} is that this class wraps
 * a seekable stream and handles seeks in a more efficient way. It will create files with holes on
 * file systems supporting such files.
 */
public class DiskCachedImageInputStream extends ImageInputStreamImpl {
  ...
  public DiskCachedImageInputStream(ImageInputStream stream, int readAheadSize, File cacheDir)
    ...
    if (cacheDir == null) {
      ...
      this.cacheFile =
          Files.createTempFile(cacheDir.toPath(), "image-stream-cache-", ".tmp").toFile();
    }
   ...
   * parts of the read request cannot be satisfied by the disk cache file, so that we can read those
   * parts from the underlying stream and fill the cache.
   *
   * @param range the range to subtract from
   * @param minusRanges the ranges to subtract. This list must be sorted.
   * @param readAheadSize add additional size to the last range if the tail of the range is not
   *     covered
   * @return a list of ByteRange objects that are not covered by the given ranges. The list is
   *     sorted
   */
  public static List<ByteRange> resolveMissingRanges(
      ByteRange range, List<ByteRange> minusRanges, int readAheadSize) {
