 ...
 *
 * <p>The resources held by this class is managed by thread-local {@link OutDbResourcePool}
 * instances. Please make sure to call {@link #dispose(boolean)} to release the resources when the
...
public class OutDbGridCoverage2D extends GridCoverage2D {
  ...
  /**
   * This is for supporting the construction of LazyLoadingGridCoverage2D. The object will be
   * initialized as a well-defined dummy state since this object won't be actually used after
   * construction. All the methods in LazyLoadingGridCoverage2D will be delegated to a real
   * OutDbGridCoverage2D object.
   */
  ...
  /**
   * Replace the placeholder image with the actual image. This method is called when the pixel data
   * of the grid coverage is needed.
   */
  ...
  public static class SerializableState implements KryoSerializable, Serializable {
    ...
    public OutDbGridCoverage2D restore(byte[] serializedConf) {
      if (serializedConf == null) {
        throw new IllegalStateException(
            "Cannot restore out-db grid coverage without configuration");
      }
      GridGeometry2D gridGeometry =
          new GridGeometry2D(gridEnvelope2D, gridToCRS, CRSSerializer.deserialize(serializedCRS));
      OutDbResourcePool.ResourceKey resourceKey =
          new OutDbResourcePool.ResourceKey(path, serializedConf, params);
  ...
  public SerializableState getSerializableState(boolean withConfiguration) {
    ...
    // Serialized configuration is pretty large (usually >= 80KB). We serialize configuration only
    // if requested.
  ...
  public static OutDbGridCoverage2D create(CharSequence name, Path path, Configuration conf)
      throws IOException {
    OutDbResourcePool.ResourceKey resourceKey = new OutDbResourcePool.ResourceKey(path, conf);
  ...
  public static OutDbGridCoverage2D create(
    ...
    try {
      ...
      return new OutDbGridCoverage2D(
          name,
          planarImage,
          sourceGrid.getGridGeometry(),
          sourceGrid.getSampleDimensions(),
          bandIndices,
          resource);
    } catch (Exception e) {
      pool.release(resource);
  ...
  private static PlanarImage createPlaceHolderImage(
      GridGeometry2D gridGeometry, int dataType, GridSampleDimension[] bands, int[] bandIndices) {
  ...
  /**
   * Calculate the grid coordinate of the origin of gridGeom1 in gridGeom0. It also checks if the
   * grid geometries of two grid coverages align with each other, and throw exception if they do not
   * match.
   *
   ...
   */
  private static GridCoordinates2D calculateImageOffset(
      GridGeometry2D gridGeom0, GridGeometry2D gridGeom1) {
    ...
    if (!CRS.equalsIgnoreMetadata(crs0, crs1)) {
      LOGGER.warning(
          String.format(
              "The grid coverages have different CRS (%s vs %s)", crs0.toWKT(), crs1.toWKT()));
    }
