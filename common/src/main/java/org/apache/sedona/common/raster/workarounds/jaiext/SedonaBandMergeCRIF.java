 ...
 * under the License.
 */
package org.apache.sedona.common.raster.workarounds.jaiext;
...
/**
 * This class is modified from the original BandMergeCRIF class in JAI-EXT to workaround <a
 * href="https://github.com/geosolutions-it/jai-ext/issues/299">issue 299</a>. The modification
 * happens in the eqTol method.
 */
public class SedonaBandMergeCRIF extends CRIFImpl {

  /** Tolerance for double equality checking */
  private static final double EPS = 1e-9;

  /** Constructor. */
  public SedonaBandMergeCRIF() {
    super("bandmergeOp");
  }
   ...
   *
   * @param paramBlock The two or more source images to be "Merged" together; if No Data are
   *     present, also a NoData Range and a double value for the destination no data are present.
  ...
  public RenderedImage create(ParameterBlock paramBlock, RenderingHints renderHints) {
    ...
    List<Object> sources = new ArrayList<>(numSources);
    ...
    if (transform != null && !transform.isEmpty() && !allIdentities(transform)) {
      ...
      return new BandMergeOpImage(
          sources, renderHints, nodata, roi, destinationNoData, setAlpha, layout);
    }
  }
  ...
  /**
   * Checks the two values are equal minus a given tolerance. This is where the modification is
   * made.
   */
