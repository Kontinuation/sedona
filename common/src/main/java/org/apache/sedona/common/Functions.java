...
public class Functions {
        ...
        for (String param : listParams) {
            ...
            if (singleParam.length != 2) {
                throw new IllegalArgumentException(
                        String.format(
                                "%s is not the valid format. The valid format is key=value, for"
                                        + " example `endcap=butt quad_segs=4`.",
                                param));
            }
            ...
            if (singleParam[0].equalsIgnoreCase(listBufferParameters[0])) {
                try {
                    ...
                    throw new IllegalArgumentException(
                            String.format(
                                    "%1$s is not an integer. Quadrant segment should be an"
                                            + " integer.",
                                    singleParam[1]));
                }
            }
            ...
            // Set mitre ratio limit
                ...
                try {
                    ...
                    throw new IllegalArgumentException(
                            String.format(
                                    "%1$s is not a double. Mitre limit should be a double.",
                                    singleParam[1]));
                }
                continue;
            }
            ...
            // everything else
                ...
                throw new IllegalArgumentException(
                        String.format(
                                "%s is not a valid style parameter. Accepted style parameters are"
                                        + " %s.",
    ...
    /**
     * Corrects the longitudes of a geometry to be within the -180 to 180 range. This method
     * modifies the original geometry.
     *
    ...
    public static Double zMax(Geometry geometry) {
        ...
        double max = -Double.MAX_VALUE;
        for (int i = 0; i < points.length; i++) {
            if (java.lang.Double.isNaN(points[i].getZ())) continue;
    ...
    public static int nDims(Geometry geometry) {
        ...
        if (!java.lang.Double.isNaN(x_cord)) count_dimension++;
        if (!java.lang.Double.isNaN(y_cord)) count_dimension++;
        if (!java.lang.Double.isNaN(z_cord)) count_dimension++;
        if (!java.lang.Double.isNaN(m_cord)) count_dimension++;
    ...
    public static Geometry addPoint(Geometry linestring, Geometry point, int position) {
        if (linestring instanceof LineString && point instanceof Point) {
            List<Coordinate> coordinates =
                    new ArrayList<>(Arrays.asList(linestring.getCoordinates()));
    ...
    public static Geometry closestPoint(Geometry left, Geometry right) {
        ...
        try {
            ...
            throw new IllegalArgumentException(
                    "ST_ClosestPoint doesn't support empty geometry object.");
        }
    }
    ...
    public static Geometry lineMerge(Geometry geometry) {
        if (geometry instanceof MultiLineString) {
            ...
            if (merger.getMergedLineStrings().size() == 1) {
                ...
                // if the merger couldn't join the lines, it will contain the individual lines, so
                // return the input
                return geometry;
            }
        }
     ...
     * return false. If the exterior ring is clockwise and the interior ring(s) are
     * counter-clockwise then returns true, otherwise false.
     *
    ...
    public static boolean isPolygonCW(Geometry geom) {
        if (geom instanceof MultiPolygon) {
            ...
            for (int i = 1; i < multiPolygon.getNumGeometries(); i++) {
                arePolygonsCW =
                        arePolygonsCW && checkIfPolygonCW((Polygon) multiPolygon.getGeometryN(i));
            }
    ...
    private static boolean checkIfPolygonCW(Polygon geom) {
        ...
        for (int i = 1; i < geom.getNumInteriorRing(); i++) {
            isInteriorRingCW =
                    isInteriorRingCW
                            && Orientation.isCCW(geom.getInteriorRingN(i).getCoordinateSequence());
        }
    ...
    /**
     * Forces a Polygon/MultiPolygon to use counter-clockwise orientation for the exterior ring and
     * a clockwise for the interior ring(s).
     *
     * @param geom
    ...
    /**
     * This function accepts Polygon and MultiPolygon, if any other type is provided then it will
     * return false. If the exterior ring is counter-clockwise and the interior ring(s) are
     * clockwise then returns true, otherwise false.
     *
     * @param geom Polygon or MultiPolygon
     * @return
     */
    ...
    public static Long[] s2CellIDs(Geometry input, int level) {
        ...
        for (Geometry geom : geoms) {
            try {
                cellIds.addAll(
                        S2Utils.s2RegionToCellIDs(
                                S2Utils.toS2Region(geom), 1, level, Integer.MAX_VALUE - 1));
            } catch (IllegalArgumentException e) {
                // the geometry can't be cast to region, we cover its coordinates, for example,
                // Point
                cellIds.addAll(
    ...
    public static Long[] h3CellIDs(Geometry input, int level, boolean fullCover) {
        ...
        for (Geometry geom : geoms) {
            if (geom instanceof Polygon) {
                ...
                cellIds.addAll(
                        H3Utils.polygonToCells((Polygon) geom.getEnvelope(), level, fullCover));
            }
        }
    ...
    public static long h3CellDistance(long cell1, long cell2) {
        ...
        if (resolution != H3Utils.h3.getResolution(cell2)) {
            throw new IllegalArgumentException(
                    "The argument cells should be of the same resolution");
        }
        try {
     ...
     * @param exactDistance: if exactDistance is true, it will only return the cells on the exact
     *     kth ring, else will return all 0 - kth neighbors
     * @return
     */
        ...
        for (int j = 0; j < h3.size(); j++) {
            for (List<List<LatLng>> shellHoles :
                ...
                if (rings.isEmpty()) {
                    ...
                    polygons.add(
                            geomFactory.createPolygon(shell, rings.toArray(new LinearRing[0])));
                }
            }
        }
    ...
    public static Geometry translate(
    ...
    public static Geometry affine(
        ...
        if (!geometry.isEmpty()) {
            GeomUtils.affineGeom(
                    geometry, a, b, null, d, e, null, null, null, null, xOff, yOff, null);
        }
        return geometry;
    }
        ...
        if (geometry.isEmpty()) {
            ...
            for (Coordinate currCoordinate : geometry.getCoordinates()) {
                startX =
                        startX == null
                                ? currCoordinate.getX()
                                : Math.min(startX, currCoordinate.getX());
                startY =
                        startY == null
                                ? currCoordinate.getY()
                                : Math.min(startY, currCoordinate.getY());
                ...
                if (is3d) {
                    ...
                    startZ =
                            startZ == null
                                    ? currCoordinate.getZ()
                                    : Math.min(startZ, currCoordinate.getZ());
                    endZ =
                            endZ == null
                                    ? currCoordinate.getZ()
                                    : Math.max(endZ, currCoordinate.getZ());
                }
            }
            ...
            return GEOMETRY_FACTORY.createLineString(
                    new Coordinate[] {startCoordinate, endCoordinate});
        }
    }
