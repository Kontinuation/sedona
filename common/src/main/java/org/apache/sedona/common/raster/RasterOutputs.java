...
public class RasterOutputs {
    ...
    private static byte[] doAsPNGSingleBand(GridCoverage2D raster, int maxWidth)
            throws IOException {
        RenderedImage image = raster.getRenderedImage();
        RenderedOp scaleOp = scaleImageToFitMaxWidth(image, maxWidth);
        if (scaleOp != null) {
                ...
                for (int x = 0; x < image.getWidth(); x++) {
                    ...
                    if (Double.compare(pixel[0], noDataValue) == 0) {
                        ...
                        outputRaster.setPixel(
                                x,
                                y,
                                new int[] {normalizedValue, normalizedValue, normalizedValue, 255});
                    }
                }
            }
        ...
        for (double value : values) {
            ...
            int preDecimal = splitByDecimal[0].length(),
                    postDecimal =
                            Math.min(
                                    decimalPrecision,
                                    splitByDecimal.length > 1 ? splitByDecimal[1].length() : 0);
    ...
    private static String createPaddedMatrixStringFromInt(int[] values, int width) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            String fmt =
                    String.format(
                            "%s%%%dd%s",
                            col == 0 ? "|" : "  ", maxColWidth, col < width - 1 ? "" : "|%n");
            res.append(String.format(fmt, values[i]));
        }
    ...
    public static String createHTMLString(GridCoverage2D raster, int imageWidth)
            throws IOException {
