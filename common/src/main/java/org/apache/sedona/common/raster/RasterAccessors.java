...
public class RasterAccessors {
   ...
   *
   * <p>TODO: currently out-db rasters only reference one single file, but in the future we may
   * support referencing different files for each band. In that case, this method should accept a
   * band index as parameter.
   *
  ...
  public static String bandPath(GridCoverage2D raster) {
    ...
    try {
      return outDbGridCoverage2D.getOutDbPath().toString();
