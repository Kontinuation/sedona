...
public class H3Utils {
    ...
    public static List<Long> polygonToCells(Polygon polygon, int level, boolean fullCover) {
        try {
            List<LatLng> shell = Arrays.stream(polygon.getExteriorRing().getCoordinates()).map(H3Utils::coordindateToLatLng).collect(Collectors.toList());
            List<List<LatLng>> holes = new ArrayList<>();
            for (int i = 0; i < polygon.getNumInteriorRing(); i++) {
                holes.add(
                        Arrays.stream(polygon.getInteriorRingN(i).getCoordinates()).map(H3Utils::coordindateToLatLng).collect(Collectors.toList())
                );
            }
            // H3 polyfill only include hexagons with centroid within the polygon, we fix by generating the neighbors for the cells.
            List<Long> internalCells = h3.polygonToCells(
                    shell,
    ...
    public static List<Long> approxPathCells(Coordinate sc, Coordinate ec, int level, boolean safe) {
        ...
        for (int it = 0;it <= itCellNum; ++it) {
            cells.add(coordinateToCell(new Coordinate(sc.getX() + xUnit * it, sc.getY() + yUnit * it), level));
        }
        ...
        try {
            ...
            for (int pairE = 1; pairE < coordinates.length; pairE++) {
                ...
                try {
                    ...
                    if (fullCover) {
                        cells.addAll(
                                polygonToCells((Polygon) (new GeometryFactory().createLineString(new Coordinate[]{cs, ce}).getEnvelope()),
                                        level,
                                        true
                                )
                        );
            ...
            throw new H3UtilException(
                    String.format("fail to cover the line %s with error %s", GeomUtils.getWKT(line), e.getMessage()),
                    e
            );
        }
    }
