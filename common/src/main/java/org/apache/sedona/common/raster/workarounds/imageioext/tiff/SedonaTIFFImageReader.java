...
public class SedonaTIFFImageReader extends TIFFImageReader {

  private static final Logger LOGGER = Logger.getLogger(SedonaTIFFImageReader.class.toString());

  /**
   * This class can be used to cache basic information about a tiff page.
   *
   * <p>Notice that we hold {@link IIOMetadata} using {@link SoftReference}s since they might be big
   * so we might want to reclaim the least used ones.
   *
  ...
  protected static final class PageInfo {
    ...
    public boolean equals(Object obj) {
      ...
      if (bigtiff != other.bigtiff) return false;
      if (!Arrays.equals(bitsPerSample, other.bitsPerSample)) return false;
      if (!Arrays.equals(colorMap, other.colorMap)) return false;
      if (compression != other.compression) return false;
      if (!Arrays.equals(extraSamples, other.extraSamples)) return false;
      if (height != other.height) return false;
      if (isImageTiled != other.isImageTiled) return false;
      if (noData == null) {
        if (other.noData != null) return false;
      } else if (!noData.equals(other.noData)) return false;
      if (numBands != other.numBands) return false;
      if (photometricInterpretation != other.photometricInterpretation) return false;
      if (planarConfiguration != other.planarConfiguration) return false;
      if (!Arrays.equals(sampleFormat, other.sampleFormat)) return false;
      if (samplesPerPixel != other.samplesPerPixel) return false;
      if (tileOrStripHeight != other.tileOrStripHeight) return false;
      if (tileOrStripWidth != other.tileOrStripWidth) return false;
      if (width != other.width) return false;
      if (!Arrays.equals(offsets, other.offsets)) return false;
      if (!Arrays.equals(scales, other.scales)) return false;
    ...
    public String toString() {
      return "PageInfo [bigtiff="
          + bigtiff
          + ", bitsPerSample="
          + Arrays.toString(bitsPerSample)
          + ", colorMap="
          + Arrays.toString(colorMap)
          + ", compression="
          + compression
          + ", extraSamples="
          + Arrays.toString(extraSamples)
          + ", height="
          + height
          + ", isImageTiled="
          + isImageTiled
  ...
  public SedonaTIFFImageReader(ImageReaderSpi originatingProvider) {
  ...
  public void setInput(Object input, boolean seekForwardOnly, boolean ignoreMetadata) {
    ...
    if (input != null) {
      ...
      if (!ImageIOUtilities.isSkipExternalFilesLookup()
          && input instanceof FileImageInputStreamExtImpl) {
  ...
  private void readHeader() throws IIOException {
    ...
    try {
      ...
      if (offset >= 0) {
        ...
        stream.seek(offset);
      } else processWarningOccurred("Error calculating offset");
    ...
    for (int i = 0; i < numImg; i++) {
      ...
      if (f != null) {
        ...
        if (data instanceof long[]) {
          ...
          numOverviews +=
              ((ldata & BaselineTIFFTagSet.NEW_SUBFILE_TYPE_REDUCED_RESOLUTION) > 0) ? 1 : 0;
          numMaskOverView +=
              ((ldata & BaselineTIFFTagSet.NEW_SUBFILE_TYPE_REDUCED_RESOLUTION) > 0
                      && (ldata & BaselineTIFFTagSet.NEW_SUBFILE_TYPE_TRANSPARENCY) > 0)
                  ? 1
                  : 0;
        }
      }
  ...
  /**
   * Method which checks if external masks or overviews are present and, if so, updates the {@link
   * DatasetLayout}
   *
   * @throws IIOException
   */
      ...
      switch (magic) {
        case 42:
          while (index < imageIndex) {
            ...
            if (offset == 0) {
              if (DEBUG) System.out.println("Offset 0 in locate");
              currIndex = index;
              imageMetadata = null;
  ...
  /**
   * Verify that imageIndex is in bounds, find the image IFD, read the image metadata, initialize
   * instance variables from the metadata.
   *
   * @param imageIndex
   * @throws IIOException
   */
  ...
  protected void prepareRead(int imageIndex, ImageReadParam param) throws IOException {
    ...
    // see if band selection with enhanced read param has been performed (allows for duplicate
    // bands)
  ...
  private static ImageTypeSpecifier getBandSelectedImageType(
      int numBands, Iterator<ImageTypeSpecifier> imageTypes) throws IIOException {
  ...
  private void decodeTile(
    ...
    Rectangle dstRect = new Rectangle(dstMinX, dstMinY, dstWidth, dstHeight);
  ...
  private void reportProgress(BufferedImage theImage) throws IOException {
    ...
    processImageUpdate(theImage, dstMinX, dstMinY, dstWidth, dstHeight, 1, 1, destinationBands);
  }
  ...
  public BufferedImage read(int imageIndex, ImageReadParam param) throws IOException {
    ...
    if (this.decompressor == null) {
      if (compression == BaselineTIFFTagSet.COMPRESSION_NONE) {
        ...
        this.decompressor = new SedonaTIFFLZWDecompressor(predictor);
  ...
  protected static BufferedImage getDestination(
    ...
    if (imageType == null) {
      if (param instanceof EnhancedImageReadParam
          && ((EnhancedImageReadParam) param).getBands() != null) {
