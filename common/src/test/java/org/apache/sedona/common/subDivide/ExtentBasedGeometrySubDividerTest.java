...
public class ExtentBasedGeometrySubDividerTest {
  ...
  public void testSubdividePolygonWithHoles() {
    ...
    Polygon polygonWithHole =
        factory.createPolygon(shell.getExteriorRing(), new LinearRing[] {hole.getExteriorRing()});
  ...
  public void testSubdivideMultiPolygon() {
    ...
    Iterator<Geometry> iter =
        divider.subdivide(
            factory.createMultiPolygon(new Polygon[] {(Polygon) polygon1, (Polygon) polygon2}));
    List<Polygon> subGeoms = new ArrayList<>();
    while (iter.hasNext()) {
      subGeoms.add((Polygon) iter.next());
    }
  ...
  public void testSubdivideGeometryCollection() {
    ...
    Geometry multiPolygon =
        factory.createMultiPolygon(
            new Polygon[] {(Polygon) factory.createPoint(new Coordinate(10, 20)).buffer(1)});
