...
public class S2UtilTest {
    ...
    public void coverPolygon() throws ParseException {
        ...
        assertEquals(
                cellIds.stream().max(Comparator.comparingLong(S2CellId::level)).get().level(), 5);
    }
    ...
    public void coverPolygonWithHole() throws ParseException {
        Polygon polygon =
                (Polygon)
                        new WKTReader()
                                .read(
                                        "POLYGON((0.5 0.5,5 0,5 5,0 5,0.5 0.5), (1.5 1,4 1,4 3,1.5"
                                                + " 1))");
        ...
        List<S2CellId> cellIds =
                S2Utils.roundCellsToSameLevel(
                        S2Utils.s2RegionToCellIDs(
                                S2Utils.toS2Polygon(polygon), 1, 8, Integer.MAX_VALUE - 1),
                        8);
        ...
        assertEquals(
                8, cellIds.stream().max(Comparator.comparingLong(S2CellId::level)).get().level());
    ...
    public void coverLineString() throws ParseException {
        ...
        assertEquals(
                cellIds.stream().max(Comparator.comparingLong(S2CellId::level)).get().level(), 8);
    }
    ...
    public void coverLinearLoop() throws ParseException {
        LineString line =
                GEOMETRY_FACTORY.createLineString(
                        new WKTReader()
                                .read("LINESTRING (1.5 2.45, 3.21 4, 5 2, 1.5 2.45)")
                                .getCoordinates());
        List<S2CellId> cellIds =
                S2Utils.s2RegionToCellIDs(S2Utils.toS2PolyLine(line), 1, 8, Integer.MAX_VALUE);
    ...
    public void toS2Loop() throws ParseException {
        ...
        LinearRing ringCCW =
                GEOMETRY_FACTORY.createLinearRing(
                        new WKTReader()
                                .read("LINESTRING (1.5 2.45, 5 2, 3.21 4, 1.5 2.45)")
                                .getCoordinates());
                        ...
                        s2Loop.vertices().stream()
                                ...
                                .map(
                                        l ->
                                                new Coordinate(
                                                        Double.parseDouble(
                                                                df.format(l.lngDegrees())),
                                                        Double.parseDouble(
                                                                df.format(l.latDegrees()))))
