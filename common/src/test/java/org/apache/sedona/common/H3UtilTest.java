...
public class H3UtilTest {
    ...
    public static Polygon[] cellsToJTSPolygons(List<Long> cells) {
        return H3Utils.h3.cellsToMultiPolygon(cells, true).stream().map(
                shellHoles -> {
                    List<LinearRing> rings = shellHoles.stream().map(
                            shell -> GEOMETRY_FACTORY.createLinearRing(shell.stream().map(latLng -> new Coordinate(latLng.lng, latLng.lat)).toArray(Coordinate[]::new))
    ...
    public void polygonOnPentagonToCells() throws ParseException {
        Polygon target = (Polygon) new WKTReader().read("POLYGON((10.47311531687734 64.74411329535432,10.427796713361715 64.67488499523719,10.543153158674215 64.64902571378299,10.674989096174215 64.68017135781035,10.595338217267965 64.76636949080041,10.47311531687734 64.74411329535432))");
    ...
    public void polygonWithHolesToCells() throws ParseException {
        Polygon target = (Polygon) new WKTReader().read("POLYGON((-122.49150256862254 37.76006117790207,-122.46506671657176 37.78394270449753,-122.41906146754832 37.77715896429647,-122.3905656789741 37.74187348046013,-122.42524127711863 37.71254658823643,-122.47811298122019 37.72666839340869,-122.49150256862254 37.76006117790207), (-122.44927386989207 37.73752948850401,-122.42798785914988 37.74920338883217,-122.42730121364207 37.76196112716144,-122.46335010280222 37.76250396084337,-122.44927386989207 37.73752948850401)))");
        List<Long> cells = H3Utils.polygonToCells(target, 9, true);
        // the cells of mbr must include all the cells' cells
        Polygon mbr = (Polygon) target.getEnvelope();
        List<Long> cellsMBR = H3Utils.polygonToCells(mbr, 9, true);
        assert new HashSet<>(cellsMBR).containsAll(cells);
        // generate the reversed polygons from cells, the h3 generate 1 compacted polygon for connected cells
        Polygon cellPolygon = cellsToJTSPolygons(cells)[0];
        // the reversed polygons from cells must all intersect with the polygon, this means the indexing geographically make sense
        assert cellPolygon.contains(target);
        // catch exception
        Exception exception = assertThrows(RuntimeException.class, () -> {
            H3Utils.polygonToCells(
                    (Polygon) new WKTReader().read("POLYGON ((75 29, 77 29, 75 29))"),
                    7,
                    true
            );
        });
    ...
    public void approxPathCells()  {
        LineString target = GEOMETRY_FACTORY.createLineString(coordArray(-122.532, 37.771, -122.589, 37.743));
        List<Long> cells = H3Utils.lineStringToCells(target, 8, false);
        List<Long> approxCells = H3Utils.approxPathCells(target.getCoordinateN(0), target.getCoordinateN(1), 8, true);
    ...
    public void approxPathCellsPentagon()  {
        LineString target = GEOMETRY_FACTORY.createLineString(coordArray(58.174758948493505, 10.427371502467615, 58.1388817207103, 10.469490838693966));
        // assert use h3 native function will throw error
        assertThrows(H3Exception.class, () -> {
        ...
        List<Long> cells = H3Utils.approxPathCells(
                target.getCoordinateN(0),
                target.getCoordinateN(1),
                10,
                true
        );
        Long pentagonCell = cells.stream().filter(H3Utils.h3::isPentagon).collect(Collectors.toList()).get(0);
        Set<Long> h3Cells = new HashSet<>(
                H3Utils.h3.gridPathCells(
                        H3Utils.coordinateToCell(target.getCoordinateN(0), 10),
                        pentagonCell
                )
        );
        h3Cells.addAll(H3Utils.h3.gridPathCells(
                pentagonCell,
                H3Utils.coordinateToCell(target.getCoordinateN(1), 10)
        ));
    ...
    public void lineStringToCells()  {
        LineString target = GEOMETRY_FACTORY.createLineString(coordArray(-122.589, 37.743, -122.532, 37.771, -122.423, 37.742, -122.323, 37.842));
    ...
    public void lineStringCrossPentagonToCells()  {
        ...
        LineString target = GEOMETRY_FACTORY.createLineString(coordArray(10.466683443818056, 64.72575037391769,10.595772799286806, 64.66235272011512));
        // first this case throw error when call native h3 functions
        assertThrows(H3Exception.class, () -> H3Utils.h3.gridPathCells(
    ...
    public void lineStringCrossPentagonToCellsNonFullCover()  {
        ...
        // generate the reversed polygons from cells, the h3 generate 1 compacted polygon for adjacent cells
        List<Long> approxCells = H3Utils.approxPathCells(target.getCoordinateN(0), target.getCoordinateN(1), 6, true);
