...
public class RasterBandEditorsTest extends RasterTestBase {
  ...
  @Test
  public void testClipEdgeCase() throws FactoryException, TransformException, ParseException {
    ...
    expectedValues =
        new double[] {
          0.0, 0.0, 64.0, 65.0, 0.0, 0.0, 0.0, 0.0, 54.0, 55.0, 56.0, 0.0, 0.0, 43.0, 44.0, 45.0,
          46.0, 47.0, 32.0, 33.0, 34.0, 35.0, 36.0, 37.0, 22.0, 23.0, 24.0, 25.0, 26.0, 27.0, 12.0,
  ...
  public void testClipOutDb() throws FactoryException, ParseException, TransformException {
    ...
    GridCoverage2D raster = new LazyLoadOutDbGridCoverage2D("test", path, conf);
    String polygon =
        "POLYGON ((-115.026855 31.353637, -115.026855 36.985003, -109.006348 36.985003, -109.006348 31.353637, -115.026855 31.353637))";
    Geometry geom = Constructors.geomFromWKT(polygon, RasterAccessors.srid(raster));
