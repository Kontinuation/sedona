...
public class DDLGenerator {
    ...
    public static void printUsage() {
        System.out.println(
                "Usage: java -jar snowflake/target/sedona-snowflake-1.5.1.jar [options]");
        ...
        System.out.println(
                "  --schema <schema>  register functions to this schema. Default to sedona");
        System.out.println(
                "  --stageName <stageName>  snowflake stage name to upload jar files. Not needed"
                        + " if isNativeApp is true");
        System.out.println(
                "  --isNativeApp <true/false>  whether to generate DDL for Snowflake Native App."
                        + " Default to false");
        System.out.println(
                "  --appRoleName <appRoleName>  application role name. Required when isNativeApp"
                        + " is true. Default to app_public");
    ...
    public static void main(String[] args) {
        ...
        // check if isNativeApp. If so, set stageName to empty string since it is not needed. Also
        // set appRoleName.
        ...
        if (argMap.getOrDefault("isNativeApp", "false").equals("true")) {
            ...
            if (!argMap.containsKey("appRoleName")) {
                System.out.println(
                        "-- AppRoleName is required when isNativeApp is true. If not provided,"
                                + " default to app_public");
            }
            stageName = "";
            ...
            System.out.println(
                    "GRANT USAGE ON SCHEMA sedona TO APPLICATION ROLE " + appRoleName + ";");
        } else {
            // If isNativeApp is false, set stageName to the provided value. Also set appRoleName to
            // empty string since it is not needed.
        ...
        try {
            ...
            System.out.println(
                    String.join(
                            "\n",
                            UDTFDDLGenerator.buildAll(
                                    argMap, stageName, isNativeApp, appRoleName)));
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
