 ...
 *
 * This rule detects LogicalRelations that use ParquetFileFormat (but NOT GeoParquetFileFormat)
 * and have nested GeometryUDT in their schema, then transforms the schema to use BinaryType
 * instead. GeoParquetFileFormat is excluded because it has its own handling for nested geometry.
 */
      ...
      case lr: LogicalRelation
          if lr.relation.isInstanceOf[HadoopFsRelation] &&
            ...
            !lr.relation
              .asInstanceOf[HadoopFsRelation]
              .fileFormat
              .isInstanceOf[GeoParquetFileFormat] &&
