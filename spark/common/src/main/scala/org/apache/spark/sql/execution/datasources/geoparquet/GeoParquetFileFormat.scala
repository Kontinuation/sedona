...
import org.apache.spark.sql.execution.datasources.geoparquet.internal.{ParquetFilters, ParquetFooterReader, ParquetOptions, ParquetOutputWriter, ParquetReadSupport, ParquetRowIndexUtil, ParquetWriteSupport, PortableSQLConf, SchemaMergeUtils, DataSourceUtils => InternalDataSourceUtils}
...
// We deliberately inherit from Spark's ParquetFileFormat instead of our own
// internal.ParquetFileFormat. This is because Spark has some magic that only works with
// its own ParquetFileFormat, such as
// automatic schema pruning for nested columns (https://github.com/apache/spark/blob/v4.0.0/sql/core/src/main/scala/org/apache/spark/sql/execution/datasources/SchemaPruning.scala#L112-L115), and
// automatically converting ColumnarBatchRow to UnsafeRow (https://github.com/apache/spark/blob/v4.0.0/sql/core/src/main/scala/org/apache/spark/sql/execution/DataSourceScanExec.scala#L631-L637).
// We don't have to support Databricks, so depending on Spark's ParquetFileFormat should be fine.
