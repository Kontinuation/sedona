...
private[apache] case class ST_ExtentBasedSubDivide(inputExpressions: Seq[Expression])
    extends InferredExpression(
...
private[apache] case class ST_ExtentBasedSubDivideExplode(children: Seq[Expression])
    extends Generator
  ...
  override def eval(input: InternalRow): TraversableOnce[InternalRow] = {
    ...
    geometryExpr.toGeometry(input) match {
      case geom: Geometry =>
        ...
        if (nArgs == 3) {
          Functions
            .extentBasedSubDivide(geom, maxWidth, maxHeight)
            .map(_.toGenericArrayData)
            .map(InternalRow(_))
        } else if (nArgs == 4) {
          val algorithms = children(3).asString(input)
          Functions
            .extentBasedSubDivide(geom, maxWidth, maxHeight, algorithms)
            .map(_.toGenericArrayData)
            .map(InternalRow(_))
        } else if (nArgs == 6) {
          ...
          Functions
            .extentBasedSubDivide(geom, maxWidth, maxHeight, maxVertices, maxDepth, algorithms)
            .map(_.toGenericArrayData)
            .map(InternalRow(_))
        } else {
...
private[apache] case class ST_XZ2(inputExpressions: Seq[Expression])
