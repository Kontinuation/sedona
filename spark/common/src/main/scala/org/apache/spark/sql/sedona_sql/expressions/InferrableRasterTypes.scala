...
object InferrableRasterTypes {
  ...
  def disposeInputRasters(inputRasters: Array[AnyRef], result: Any): Unit = {
    result match {
      case result: GridCoverage2D =>
        inputRasters.foreach { input =>
          if (input ne result) Try(input.asInstanceOf[GridCoverage2D].dispose(true))
        }
      case results: Array[GridCoverage2D] =>
        inputRasters.foreach { input =>
          if (!results.exists(_ eq input)) Try(input.asInstanceOf[GridCoverage2D].dispose(true))
        }
      case _ =>
        inputRasters.foreach(input => Try(input.asInstanceOf[GridCoverage2D].dispose(true)))
    }
  }

  def disposeInputRasters(inputRasters: Array[AnyRef]): Unit = {
    inputRasters.foreach(input => Try(input.asInstanceOf[GridCoverage2D].dispose(true)))
  }
}
