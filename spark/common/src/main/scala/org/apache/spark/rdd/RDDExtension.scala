...
object RDDExtension {
  ...
  /**
   * A variant of RDD.zipPartitions that allows the function to take the index of the partition as
   * parameter. This is for supporting adaptive index lookup in advanced spatial join.
   */
  def zipPartitionsWithIndex[T: ClassTag, B: ClassTag, V: ClassTag](
      rdd1: RDD[T],
      rdd2: RDD[B],
      preservesPartitioning: Boolean)(f: (Int, Iterator[T], Iterator[B]) => Iterator[V]): RDD[V] =
    rdd1.withScope {
   ...
   * partition as parameter. This is for supporting adaptive index lookup in advanced spatial
   * join.
   */
  def javaZipPartitionsWithIndex[T, U, V](
    ...
    val resultRdd = zipPartitionsWithIndex(rdd.rdd, other.rdd, preservesPartitioning = false)(fn)(
      rdd.classTag,
      other.classTag,
      fakeClassTag[V])
