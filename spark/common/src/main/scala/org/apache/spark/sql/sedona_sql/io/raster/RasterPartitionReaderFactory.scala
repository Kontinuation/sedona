...
case class RasterPartitionReaderFactory(
  ...
  private def buildReader(
    ...
    val fileReader = if (loadRasterMetadata && rasterLoadingParallelism > 0) {
      new RasterParallelPartitionReader(
  ...
  override def createReader(partition: InputPartition): PartitionReader[InternalRow] = {
    partition match {
      case filePartition: RasterInputPartition => buildReader(filePartition.files)
      case _ =>
