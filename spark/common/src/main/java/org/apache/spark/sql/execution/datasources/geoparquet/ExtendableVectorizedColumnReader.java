...
import org.apache.spark.sql.execution.datasources.geoparquet.internal.ParquetDictionary;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.ParquetReadState;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.ParquetVectorUpdater;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.ParquetVectorUpdaterFactory;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedColumnReader;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedDeltaBinaryPackedReader;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedDeltaByteArrayReader;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedDeltaLengthByteArrayReader;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedPlainValuesReader;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedRleValuesReader;
 ...
 *
 * <p>This is a copy of the VectorizedColumnReader with minor modifications to allow for it to be
 * extended to work with other UDT types.
 */
public class ExtendableVectorizedColumnReader extends VectorizedColumnReader {
  ...
  /**
   * The index for the first row in the current page, among all rows across all pages in the column
   * chunk for this reader. If there is no column index, the value is 0.
   */
  ...
  // Modification: pass in a ParquetVectorUpdater
  private final ParquetVectorUpdater updater;
  private final DataType sparkType;

  public ExtendableVectorizedColumnReader(
      ...
      ParquetVectorUpdaterFactory updaterFactory,
      DataType sparkType)
      throws IOException {
    super(
        descriptor,
    ...
    this.readState =
        new ParquetReadState(descriptor, isRequired, pageReadStore.getRowIndexes().orElse(null));
  ...
  /**
   * Checks if lazy decoding is supported for the given type.
   *
   * <p>This function has been modified from the original VectorizedColumnReader to ensure that lazy
   * decoding is not attempted for geometry types.
   *
   * @param parquetTypeName The parquet type name.
   */
  protected boolean isLazyDecodingSupported(
      PrimitiveType.PrimitiveTypeName parquetTypeName, DataType datatype) {
  ...
  public void readBatch(
    ...
    // Modification: If this is a geo type, use that to get the updater. Not all user defined
    // types are supported with updaters, so default to the column's data type for all
    // other spark types.
    ...
    while (readState.rowsToReadInBatch > 0 || !readState.lastListCompleted) {
      ...
      if (isCurrentPageDictionaryEncoded) {
        ...
        if (column.hasDictionary()
            || (startRowId == pageFirstRowIndex && isLazyDecodingSupported(typeName, sparkType))) {
          ...
          boolean castLongToInt =
              typeAnnotation instanceof DecimalLogicalTypeAnnotation
          ...
          updater.decodeDictionaryIds(
              readState.valueOffset - startOffset, startOffset, column, dictionaryIds, dictionary);
        }
      } else {
