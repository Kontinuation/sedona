...
import org.apache.spark.sql.execution.datasources.geoparquet.internal.ParquetColumnVector;
...
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedColumnReader;
import org.apache.spark.sql.execution.datasources.geoparquet.internal.VectorizedParquetRecordReader;
...
/**
 * This is a copy of VectorizedParquetRecordReader with some modifications to handle Geometry
 * columns. Modifications include creating GeoVectorizedColumnVector and keeping track of the types
 * (including geometry types) when reading the batch and passing that along to the
 * GeoVectorizedColumnReader. All changes are marked with a *Modification* comment.
 */
public class GeoVectorizedParquetRecordReader extends VectorizedParquetRecordReader {
  ...
  /**
   * For each leaf column, if it is in the set, it means the column is missing in the file and we'll
   ...
   * batched decoding. It is not valid to interleave calls to the batched interface with the row by
   * row RecordReader APIs. This is only enabled with additional flags for development. This is
   ...
   *
   * <p>When this is set, the code will branch early on in the RecordReader APIs. There is no shared
   * code between the path that uses the MR decoders and the vectorized ones.
   *
   * <p>TODOs: - Implement v2 page formats (just make sure we create the correct decoders).
   */
  private ColumnarBatch columnarBatch;

  /** Modification: Stores the schema of the batch */
  ...
  /** The spark schema that includes the geo column types */
  private final StructType geoSparkSchema;

  public GeoVectorizedParquetRecordReader(
      StructType geoSparkSchema,
      ZoneId convertTz,
    ...
    this.geoSparkSchema = geoSparkSchema;
  }
  ...
  public void initialize(InputSplit inputSplit, TaskAttemptContext taskAttemptContext)
    ...
    initializeInternal();
    this.sparkSchema = this.geoSparkSchema;
  }
  ...
  private void initBatch(
    ...
    /* Modification: Store the batch schema instead of having a local variable so
    that it is available later reading the batch */
    this.batchSchema = new StructType();
    ...
    for (int i = 0; i < columnVectors.length; i++) {
      ...
      columnVectors[i] =
          new ParquetColumnVector(
              parquetColumn.children().apply(i),
  ...
  /**
   * Returns the ColumnarBatch object that will be used for all rows returned by this reader. This
   * object is reused. Calling this enables the vectorized reader. This should be called before any
   * calls to nextKeyValue/nextBatch.
   */
  ...
  private void initializeInternal() throws IOException, UnsupportedOperationException {
    ...
    /** Modification: Use the GeoSpark schema instead of the original schema */
    this.sparkSchema = this.geoSparkSchema;
  }
  ...
  private void checkEndOfRowGroup() throws IOException {
    ...
    /* Modification: use the types from the batch schema to pass along
     * the spark type including geometry UDT types to the GeoVectorizedColumnReader.
     */
    StructField[] fields = this.batchSchema.fields();
    for (int i = 0; i < columnVectors.length; ++i) {
      initColumnReader(pages, columnVectors[i], fields[i].dataType());
    }
    totalCountLoadedSoFar += pages.getRowCount();
  }

  private void initColumnReader(PageReadStore pages, ParquetColumnVector cv, DataType sparkType)
      throws IOException {
    if (!missingColumns.contains(cv.getColumn())) {
      if (cv.getColumn().isPrimitive()) {
        ParquetColumn column = cv.getColumn();
        ColumnDescriptor desc = column.descriptor().get();
        // Modification: use the GeoVectorizedColumnReader
        GeoParquetVectorUpdaterFactory updaterFactory =
            new GeoParquetVectorUpdaterFactory(
                ...
                int96RebaseMode,
                int96RebaseTz);
        // Pass in both the spark type (with Geo UDT) and limited spark types that the lower readers
        // expect
        ParquetVectorUpdater updater =
            updaterFactory.getUpdater(desc, sparkType, column.sparkType());

        VectorizedColumnReader reader =
            new GeoVectorizedColumnReader(
                desc,
                column.required(),
                pages,
                convertTz,
        ...
        for (ParquetColumnVector childCv : cv.getChildren()) {
          initColumnReader(pages, childCv, sparkType);
        }
      }
    }
  }
  ...
  /**
   * <b>This method assumes that all constant column are at the end of schema and
   * `constantColumnLength` represents the number of constant column.<b/>
   *
   * <p>This method allocates columns to store elements of each field of the schema, the data
   * columns use `OffHeapColumnVector` when `useOffHeap` is true and use `OnHeapColumnVector` when
   * `useOffHeap` is false, the constant columns always use `ConstantColumnVector`.
   *
   * <p>Capacity is the initial capacity of the vector, and it will grow as necessary. Capacity is
