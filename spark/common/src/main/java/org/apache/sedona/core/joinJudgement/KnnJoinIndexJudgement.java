 ...
 * It extends the JudgementBase class and implements the FlatMapFunction2 interface.
 *
        ...
        implements FlatMapFunction2<Iterator<T>, Iterator<SpatialIndex>, Pair<U, T>>, Serializable
{
     ...
     */
    public KnnJoinIndexJudgement(int k, DistanceMetric distanceMetric,
     ...
     * It iterates over the geometries in the stream side and uses the spatial index to find the k nearest neighbors for each geometry.
     * The method returns an iterator over the join results.
     *
            ...
            throws Exception
    {
        ...
        while (streamShapes.hasNext()) {
            ...
            switch (distanceMetric) {
                case EUCLIDEAN:
                    localK = strTree.nearestNeighbour(streamShape.getEnvelopeInternal(), streamShape, new EuclideanItemDistance(), k);
                    break;
                case HAVERSINE:
                    localK = strTree.nearestNeighbour(streamShape.getEnvelopeInternal(), streamShape, new HaversineItemDistance(), k);
                    break;
                case SPHEROID:
                    localK = strTree.nearestNeighbour(streamShape.getEnvelopeInternal(), streamShape, new HaversineItemDistance(), k);
                    break;
                default:
                    localK = strTree.nearestNeighbour(streamShape.getEnvelopeInternal(), streamShape, new GeometryItemDistance(), k);
                    break;
            }
