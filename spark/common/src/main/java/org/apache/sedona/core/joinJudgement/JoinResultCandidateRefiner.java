...
/**
 * Create a Refiner for refining the join result candidates. To understand what a refiner do, we'll
 * take a brief review of the join process on each partition. The join process on each partition
 * takes two sets of geometries as input, one is the stream side, and the other one is build side.
 * The geometries on the build side could be restructured for efficient MBR based spatial query, or
 * simply leave it as a flat list (nested loop join). The join process is:
 *
 * <ol>
 *   <li>For each geometry on the stream side, find all candidates on the build side that may
 *       satisfy the spatial predicate.
 *   <li>For each candidate, evaluate the spatial predicate to determine whether the pair of
 *       geometries actually satisfy the spatial predicate.
 * </ol>
 *
 * The refiner is for the second step. It takes a geometry on the stream side, and a list of
 * candidate geometries retrieved from the build side, and returns a list of pairs of geometries
 * that actually satisfy the spatial predicate. The stream side can be either side of joined RDDs,
 * and we can also choose to prepare the stream side geometry to speed up evaluating the spatial
 * predicate. These many combinations leads to various implementations of the refiner.
 */
public class JoinResultCandidateRefiner {
  public interface Refiner {
    <T, U> void refine(
        Geometry streamGeometry, List<Geometry> candidateGeometries, List<Pair<T, U>> results);
  }
  ...
  public static Refiner create(boolean streamLeft, SpatialPredicate spatialPredicate) {
    ...
    if (streamLeft) {
      // Prepared geometries does not always speed up the evaluation of spatial predicates. Only
      // intersects,
      // contains and covers have an optimized implementation for prepared geometries in JTS, while
      // other
      // predicates such as within, covered_by and overlaps simply fallback to the non-prepared
      // implementation.
      // We'll have some extra overhead for constructing prepared geometries without getting any
      // speed-ups when
      // evaluating such spatial predicates. So we'll only enable prepared geometries for
      // intersects, contains and
      ...
      // Similarly, when stream geometry is on the right side, we'll only benefit from prepared
      // geometries when
      // evaluating intersects, within and covered_by. For other predicates, we'll simply use the
      // non-prepared
      // implementation.
      if (spatialPredicate == SpatialPredicate.INTERSECTS
