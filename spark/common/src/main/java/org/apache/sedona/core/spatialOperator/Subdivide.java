...
public class Subdivide {
    ...
    public static class SubdivideSpatialJoinOptions {
        ...
        public SubdivideSpatialJoinOptions(SubdivideRDDOptions globalOptions, SubdivideOptions localOptions) {
    ...
    /**
     * Determine the subdivision options for both spatial RDDs. This is done by applying some heuristics to the
     * statistics of the geometries in the spatial RDDs.
     *
     * <p>NOTE: we implement this method for processing 2 SpatialRDDs at the same time instead of only implementing
     * it for one SpatialRDD. This is because we may want to implement some complex heuristics which requires the
     * statistics of both SpatialRDDs. The subdivide options may also tangle with each other, for example, keepUserData
     * option can only be turned on when both sides are accurate. This makes it necessary to determine the options
     * given both SpatialRDDs.
     *
     * @param leftRDD Left joined Spatial RDD. This SpatialRDD should be analyzed using the {@link SpatialRDD#advancedAnalyze()}
     *                   method.
     * @param rightRDD Right joined Spatial RDD. This SpatialRDD should be analyzed using the {@link SpatialRDD#advancedAnalyze()}
     ...
     * @param <T> The type of geometry in the spatial RDD.
     */
    public static <T extends Geometry> Pair<SubdivideSpatialJoinOptions, SubdivideSpatialJoinOptions> determineSubdivideOptions(
            SpatialRDD<T> leftRDD, SpatialRDD<T> rightRDD, SpatialPartitioner spatialPartitioner,
            boolean canDiscardLeftGeometry, boolean canDiscardRightGeometry,
            SedonaConf sedonaConf) throws Exception {
        if (leftRDD.getStatistics() == null) {
            throw new IllegalArgumentException("The left spatial RDD must be analyzed using the advancedAnalyze method");
        }
        if (rightRDD.getStatistics() == null) {
            throw new IllegalArgumentException("The right spatial RDD must be analyzed using the advancedAnalyze method");
        }

        SubdivideSpatialJoinOptions leftOptions = determineSubdivideOptions(
                leftRDD, spatialPartitioner, canDiscardLeftGeometry, sedonaConf, "left");
        SubdivideSpatialJoinOptions rightOptions = determineSubdivideOptions(
        ...
        if (!(isLeftSideAccurate && isRightSideAccurate)) {
            // We cannot keep user data if either side is inaccurate, since we need the original geometry to recover
            ...
            if (leftOptions.globalOptions != null && leftOptions.globalOptions.keepUserData) {
                ...
                LOGGER.info("Not both sides are accurate after subdividing, keepUserData is turned off for the left side");
            }
            if (rightOptions.globalOptions != null && rightOptions.globalOptions.keepUserData) {
                rightOptions.globalOptions.keepUserData = false;
                LOGGER.info("Not both sides are accurate after subdividing, keepUserData is turned off for the right side");
            }
        }
    ...
    private static <T extends Geometry> SubdivideSpatialJoinOptions determineSubdivideOptions(
            SpatialRDD<T> spatialRDD, SpatialPartitioner spatialPartitioner, boolean canDiscardGeometry,
            SedonaConf sedonaConf, String rddName) throws Exception {
        ...
        SpatialPartitionStats spatialPartitionQuality = evaluateSpatialPartitionQuality(spatialPartitioner, stat);
        if (spatialPartitionQuality == null) {
            // Don't subdivide if there's no samples collected.
            return new SubdivideSpatialJoinOptions(null, null);
        }

        // Step 2: Calculate the duplication factor of the sampled envelopes and estimate shuffle write size.
        ...
        long totalGeomSize = (long) (stat.getEstimatedSizeWithoutUserDataInBytes() * inBoundCount * duplicationFactor);
        ...
        // Step 3: Consider the largest geometries, if the shuffle write size is too large, we should subdivide
        // before spatial partitioning
        LargeGeomStats largestDupByArea = calculateLargeGeomStats(stat.getTopAreaInfos(), spatialPartitioner);
        LargeGeomStats largestDupByWidth = calculateLargeGeomStats(stat.getTopWidthInfos(), spatialPartitioner);
        LargeGeomStats largestDupByHeight = calculateLargeGeomStats(stat.getTopHeightInfos(), spatialPartitioner);
        ...
        if (largestDup.numDuplicates >= duplicationFactorThreshold &&
            ...
            // Duplication of large geometries caused large amount of data to be shuffled. We should subdivide before
        ...
        // Step 4: Calculate the size of sampled envelopes, if it is large compared to the partitioning grid size and
        // has a high probability to intersect with each other, we should subdivide in local join phase.
        // NOTICE: If global subdivide is used, polygons are subdivided to boxes, there's no point to subdivide them again.
        ...
        double nonPolygonalCollisionFactorThreshold = sedonaConf.getSubdivideNonPolygonalCollisionFactorThreshold();
        double extentSizeRatioThreshold = sedonaConf.getSubdivideExtentSizeRatioThreshold();
        if (dominantGeometryType == GeometryType.POINT || dominantGeometryType == GeometryType.LINESTRING) {
            // We apply a more aggressive subdivision strategy for multipoint and linestring geometries, since they
            // occupy no space but have large envelope size, so they are more likely to produce false-postive
            // join result candidates.
            collisionFactorThreshold = Math.min(collisionFactorThreshold, nonPolygonalCollisionFactorThreshold);
        }
        if (!globalSubdivide) {
            if (meanCollisionFactor >= collisionFactorThreshold && meanExtentSizeRatio >= extentSizeRatioThreshold) {
                double polygonRatio = (double) stat.getPolygonalCount() / stat.getCount();
                if (polygonRatio >= 0.1) {
                    // Apply a more conservative subdivision strategy for dataset containing a large number of polygons,
                    // since polygons are very costy to subdivide, and they are less likely to produce false-positive
                    // join result candidates.
                    ...
                    // Apply a more aggressive subdivision strategy for non-polygonal geometries, since they are
                    // easier to subdivide, and they are more likely to produce false-positive join result candidates.
                    double factor = 0.1;
                    double localSubdivideWidth = stat.getMeanEnvelopeWidth() * factor;
                    double localSubdivideHeight = stat.getMeanEnvelopeHeight() * factor;
        ...
        // Step 5: Determine if it is possible to keep user data. Please note that keepUserData may be finally set to
        // false even though it is set to true here, since the other side may be inaccurate after being subdivided.
        ...
        if (globalSubdivide) {
            ...
            // Take size of non-geometry user data into consideration. If the user data is pretty large compared to the
            // size of discarded geometry, we should not keep user data to reduce the shuffle write size caused by
            // duplicating user data.
            long duplicatedUserDataSize = (long) (stat.getEstimatedUserDataSizeInBytes() * inBoundCount * duplicationFactor);
        ...
        LOGGER.info("Estimated element size: {}, Geometry size: {}, User data size: {}",
        ...
        LOGGER.info("numPuntal: {}, numMultiPoint: {}, numLineal: {}, numPolygonal: {}",
                stat.getPuntalCount(), stat.getMultiPointCount(), stat.getLinealCount(), stat.getPolygonalCount());
        LOGGER.info("Partitioned space: {}", boundary);
        LOGGER.info("Mean envelope size: width = {}, height = {}", stat.getMeanEnvelopeWidth(), stat.getMeanEnvelopeHeight());
    ...
    private static class SpatialPartitionStats {
        ...
        SpatialPartitionStats(List<Envelope> grids, List<EnvelopeStat> envelopeStats, long[] perPartitionSampleCount,
                              double partitionedPercentage, double duplicationFactor, double meanCollisionFactor,
                              double meanExtentSizeRatio) {
            this.grids = grids;
    ...
    private static SpatialPartitionStats evaluateSpatialPartitionQuality(
        ...
        double duplicationFactor = (samplesPartitioned > 0? (double) totalDuplicates / samplesPartitioned: 1.0);
        ...
        return new SpatialPartitionStats(grids, envelopeStats, perPartitionSampleCount,
                samplesPartitioned / (double) samples.size(),
                duplicationFactor, meanCollisionFactor, meanExtentSizeRatio);
    }
    ...
    private static LargeGeomStats calculateLargeGeomStats(
            ...
            SpatialPartitioner spatialPartitioner) throws Exception {
        ...
        for (AdvancedStatCollector.LargeGeometryInfo info : largeGeometryInfos) {
            Iterator<Tuple2<Integer, Geometry>> iterator = spatialPartitioner.placeObject(factory.toGeometry(info.extent));
