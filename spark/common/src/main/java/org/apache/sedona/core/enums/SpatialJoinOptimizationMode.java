...
public enum SpatialJoinOptimizationMode {
    /**
     * Don't optimize spatial joins, just leave them as they are (cartesian join or broadcast nested
     * loop join).
     */
    NONE,

    /**
     * Optimize all spatial joins, even though the join is an equi-join. For example, for a range
     * join like this:
     *
     ...
     * <p>{@code SELECT * FROM A, B WHERE A.x = B.x AND ST_Contains(A.geom, B.geom)}
     *
     * <p>It won't be optimized as a spatial join, since it is an equi-join (with equi-condition:
     * {@code A.x = B.x}), and could be executed by a sort-merge join or hash join.
     */
    NONEQUI;
