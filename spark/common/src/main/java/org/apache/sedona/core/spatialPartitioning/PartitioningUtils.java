...
public abstract class PartitioningUtils {
    ...
    // Check the geom against the partition tree to find the ids of overlapping grids. Only return
    // IDs
                    ...
                    try {
                        quadtreePartitioning =
                                new QuadtreePartitioning(
                                        new ArrayList<>(), paddedBoundary, resolution, minLevel);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
            ...
            case QUADTREE:
                {
                    ...
                    try {
                        quadtreePartitioning =
                                new QuadtreePartitioning(
                                        sampleEnvelopes, paddedBoundary, resolution);
                    } catch (Exception e) {
                        e.printStackTrace();
                    }
                    partitioner = quadtreePartitioning.getPartitionTree();
            ...
            case KDBTREE:
                {
                    final KDB tree =
                            new KDB(
                                    sampleEnvelopes.size() / resolution,
                                    resolution,
                                    paddedBoundary);
                ...
                try {
                    throw new Exception(
                            "[AbstractSpatialRDD][spatialPartitioning] Unsupported spatial"
                                    + " partitioning method. The following partitioning methods are"
                                    + " not longer supported: R-Tree, Hilbert curve, Voronoi");
                } catch (Exception e) {
                    e.printStackTrace();
                    break;
                }
