...
public class JoinQuery {
  ...
  public static <U extends Geometry, T extends Geometry> JavaPairRDD<U, List<T>> KNNJoinQuery(
    ...
    final JavaPairRDD<U, T> joinResults =
        knnJoin(queryRDD, objectRDD, joinParams, false, KNNJoinBroadcastSide.NONE, sedonaConf);
    return collectGeometriesByKey(joinResults);
  }
        ...
        if (leftRDD.getStatistics() != null && rightRDD.getStatistics() != null) {
          ...
          AdaptiveIndexLookupJudgement<U, T> judgement =
              new AdaptiveIndexLookupJudgement<>(
                  ...
                  partitionMaxX,
                  partitionMaxY,
                  joinParams.buildCount,
                  joinParams.streamCount,
                  joinParams.resultCount,
                  joinParams.candidateCount,
                  joinParams.buildTime,
                  joinParams.buildLeftTasks,
                  joinParams.buildRightTasks,
                  joinParams.prepareBuildTasks,
                  joinParams.prepareStreamTasks,
                  joinParams.localSubdivideLeftOptions,
                  joinParams.localSubdivideRightOptions);
  ...
  public static <U extends Geometry, T extends Geometry> JavaPairRDD<U, T> knnJoin(
      ...
      boolean includeTies,
      KNNJoinBroadcastSide broadcastSide,
      SedonaConf sedonaConf)
      throws Exception {
    verifyCRSMatch(queryRDD, objectRDD);
    if (broadcastSide == KNNJoinBroadcastSide.NONE) {
    ...
    if (broadcastSide == KNNJoinBroadcastSide.QUERY_SIDE) {
      ...
      for (U queryObject : queryRDD.rawSpatialRDD.collect()) {
        ...
        // join
        uniqueQueryObjects.add(new UniqueGeometry<>(id++, queryObject));
      }
    ...
    } else if (broadcastSide == KNNJoinBroadcastSide.OBJECT_SIDE) {
   ...
   *
   * <p>This function performs a K-Nearest Neighbors (KNN) join operation where the query geometries
   * are broadcasted to all partitions of the object geometries.
   *
   * <p>The function first maps partitions of the indexed raw RDD to perform the KNN join, then
  ...
  public static final class JoinParams {
    ...
    // SQL metrics to be used for updating metrics shown on the SQL execution page when running
    ...
    public JoinParams(
        ...
        DistanceMetric distanceMetric,
        Double searchRadius,
        SQLMetric buildCount,
        SQLMetric streamCount,
        SQLMetric resultCount,
        SQLMetric candidateCount,
        SQLMetric buildTime,
        SQLMetric buildLeftTasks,
        SQLMetric buildRightTasks,
        SQLMetric prepareBuildTasks,
        SQLMetric prepareStreamTasks,
        SubdivideOptions localSubdivideLeftOptions,
