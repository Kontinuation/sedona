...
public class JoinQuery {
    ...
    private static <U extends Geometry, T extends Geometry> void verifyCRSMatch(
        ...
        // Check CRS information before doing calculation. The two input RDDs are supposed to have
        // the same EPSG code if they require CRS transformation.
        if (spatialRDD.getCRStransformation() != queryRDD.getCRStransformation()) {
            throw new IllegalArgumentException(
                    "[JoinQuery] input RDD doesn't perform necessary CRS transformation. Please"
                            + " check your RDD constructors.");
        }

        if (spatialRDD.getCRStransformation() && queryRDD.getCRStransformation()) {
            if (!spatialRDD.getTargetEpgsgCode().equalsIgnoreCase(queryRDD.getTargetEpgsgCode())) {
                throw new IllegalArgumentException(
                        "[JoinQuery] the EPSG codes of two input RDDs are different. Please check"
                                + " your RDD constructors.");
            }
        }
    }
    ...
    private static <U extends Geometry, T extends Geometry> void verifyPartitioningMatch(
        ...
        Objects.requireNonNull(
                ...
                "[JoinQuery] spatialRDD SpatialPartitionedRDD is null. Please do spatial"
                        + " partitioning.");
        Objects.requireNonNull(
                queryRDD.spatialPartitionedRDD,
                "[JoinQuery] queryRDD SpatialPartitionedRDD is null. Please use the spatialRDD's"
                        + " grids to do spatial partitioning.");
        ...
        if (spatialNumPart != queryNumPart) {
            throw new IllegalArgumentException(
                    "[JoinQuery] numbers of partitions in queryRDD and spatialRDD don't match: "
                            ...
                            + " vs. "
                            + spatialNumPart
                            + ". Please make sure they both use the same partitioning otherwise"
                            + " wrong results will appear.");
        }
    }

    private static <U extends Geometry, T extends Geometry>
            JavaPairRDD<U, List<T>> collectGeometriesByKey(JavaPairRDD<U, T> input) {
        return input.groupBy(t -> GeomUtils.hashCode(t._1))
                .values()
                .<U, List<T>>mapToPair(
                        t -> {
    ...
    private static <U extends Geometry, T extends Geometry>
            JavaPairRDD<U, Long> countGeometriesByKey(JavaPairRDD<U, T> input) {
     ...
     * intersect. Otherwise, returns pairs of geometries where first geometry covers second
     * geometry.
     *
     * <p>If {@code useIndex} is false, the join uses nested loop algorithm to identify matching
     * geometries.
    ...
    public static <U extends Geometry, T extends Geometry> JavaPairRDD<U, T> SpatialJoinQueryFlat(
            ...
            SpatialPredicate spatialPredicate)
            throws Exception {
        final JoinParams params =
                new JoinParams(useIndex, SpatialPredicate.inverse(spatialPredicate));
        return spatialJoin(queryRDD, spatialRDD, params);
    }

    /**
     * Inner joins two sets of geometries on 'covers' or 'intersects' relationship. Results are put
     * in a flat pair format.
     *
     * <p>If {@code considerBoundaryIntersection} is {@code true}, returns pairs of geometries which
     * intersect. Otherwise, returns pairs of geometries where first geometry covers second
     * geometry.
    ...
    /**
     * {@link #SpatialJoinQueryFlat(SpatialRDD, SpatialRDD, boolean, SpatialPredicate)} count by
     * key.
     *
     * <p>Duplicate geometries present in the input queryWindowRDD RDD, regardless of their
     * non-spatial attributes, will not be reflected in the join results. Duplicate geometries
     * present in the input spatialRDD RDD, regardless of their non-spatial attributes, will be
     * reflected in the join results.
     *
     * @param <U> Type of the geometries in queryWindowRDD set
     * @param <T> Type of the geometries in spatialRDD set
     * @param spatialRDD Set of geometries
     ...
     *     otherwise
     * @return the result of {@link #SpatialJoinQueryFlat(SpatialRDD, SpatialRDD, boolean,
     *     boolean)}, but in this pair RDD, each pair contains a geometry and the count of matching
     *     geometries
     * @throws Exception the exception
     */
    @Deprecated
    ...
    public static <T extends Geometry> JavaPairRDD<Geometry, T> DistanceJoinQueryFlat(
        ...
        if (spatialPredicate != SpatialPredicate.COVERED_BY
            ...
            throw new IllegalArgumentException(
                    "Spatial predicate for distance join should be one of INTERSECTS and"
                            + " COVERED_BY");
        }
        final JoinParams joinParams =
                new JoinParams(useIndex, SpatialPredicate.inverse(spatialPredicate));
    ...
    /**
     * Inner joins two sets of geometries on 'coveredBy' relationship (aka. distance join). The
     * query window objects are converted to circle objects. The radius is the given distance.
     * Eventually, the original window objects are recovered and outputted.
     *
     * <p>If {@code considerBoundaryIntersection} is {@code true}, returns pairs of circle/geometry
     * which intersect. Otherwise, returns pairs of geometries where first circle covers second
     ...
     * @param spatialPredicate Spatial predicate in join condition {@code spatialRDD
     *     <spatialPredicate> queryRDD}, should be one of {@code INTERSECTS} and {@code COVERED_BY}
     * @return the result of {@link #DistanceJoinQueryFlat(SpatialRDD, CircleRDD, boolean,
     *     boolean)}, but in this pair RDD, each pair contains a geometry and the count of matching
     *     geometries
     * @throws Exception the exception
     */
    ...
    /**
     * Note: INTERNAL FUNCTION. API COMPATIBILITY IS NOT GUARANTEED. DO NOT USE IF YOU DON'T KNOW
     * WHAT IT IS.
     */
        ...
        if (joinParams.useIndex) {
            if (rightRDD.indexedRDD != null) {
                ...
                DynamicIndexLookupJudgement judgement =
                        new DynamicIndexLookupJudgement(
                                ...
                                candidateCount);
                joinResult =
                        leftRDD.spatialPartitionedRDD.zipPartitions(
            ...
            NestedLoopJudgement judgement =
                    new NestedLoopJudgement(
                            ...
                            candidateCount);
            joinResult =
                    rightRDD.spatialPartitionedRDD.zipPartitions(
                            leftRDD.spatialPartitionedRDD, judgement);
        }

        return joinResult
                .mapPartitionsWithIndex(
                        new DuplicatesFilter(new JavaSparkContext(cxt).broadcast(dedupParams)),
                        false)
                .mapToPair(
                        (PairFunction<Pair<U, T>, U, T>)
