...
public class ZOrderPartitioning implements Serializable {
    ...
    // correction factor for Z-order range expansion due to ordering differences between Z-order and Euclidean distance
    ...
    /**
     * Constructor to initialize ZOrderPartitioning with the given samples, boundary, and number of partitions.
     *
     ...
     *
     * Algorithm Reference:
     * Zhang, Chi, Feifei Li, and Jeffrey Jestes, Efficient parallel kNN joins for large data in MapReduce.
     * In Proceedings of the 15th international conference on extending database technology, pp. 38-49. 2012.
     *
     ...
     */
    public List<Range<Long>> createZOrderRanges(List<Envelope> samples, int neighborSampleNumber, double samplingProbability) {
        if (numPartitions <= 0) {
        ...
        List<Long> zOrderValues = samples.stream()
    ...
    public int calculateScaleFactorForRange(Envelope boundary) {
        ...
        // The scale factor should cover the largest range to ensure all coordinates fit within the int32 range
    ...
    /**
     * The provided code interleaves the bits of two integers, x and y, to generate a Z-order value (Morton code).
     * It uses the full bit length of the integers (typically 32 bits for int in Java).
     *
     * Bit Interleaving: The code interleaves the bits of x and y:
     * For each bit position i (from 0 to 31, since Integer.SIZE is 32):
     * The i-th bit of x is shifted to position 2 * i in the result z.
     * The i-th bit of y is shifted to position 2 * i + 1 in the result z.
     *
     * Grid Size: The grid size that this code can handle depends on the bit length of the coordinates:
     * Since x and y are 32-bit integers, the code can handle coordinates ranging from 0 to 2^31 − 1
     * This corresponds to a grid with side length up to 2^31 -1 in each dimension.
     *
     * @param x
     * @param y
     * @return
