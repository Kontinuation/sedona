...
public class RDDSampleUtils {
    ...
    /**
     * Returns the number of samples to take to partition the RDD into specified number of
     * partitions.
     *
     ...
     * @throws IllegalArgumentException if requested number of samples exceeds total number of
     *     records or if requested number of partitions exceeds half of total number of records
     */
    public static int getSampleNumbers(
        ...
        return (int)
                Math.max(minSampleCnt, Math.min(totalNumberOfRecords / 100, Integer.MAX_VALUE));
    }
}
