        ...
        for (int i = 0; i < paths.length; ++i) {
            ...
            if (suffix.equals(SHP_SUFFIX)) {
                shpSplit =
                        new FileSplit(
                                paths[i],
                                fileSplit.getOffset(i),
                                fileSplit.getLength(i),
        ...
        while (hasNextShp && !curShapeType.isSupported()) {
            logger.warn(
                    "[SEDONA] Shapefile type "
                            + curShapeType.name()
                            + " is not supported. Skipped this record. Please use QGIS or"
                            + " GeoPandas to convert it to a type listed in ShapeType.java");
            if (hasDbf) {
                hasNextDbf = dbfFileReader.nextKeyValue();
            }
