...
class VectorObjectCountComparator implements Comparator<Tuple2<Object, Double>>, Serializable {
    ...
    @Override
    public int compare(
            Tuple2<Object, Double> spatialObject1, Tuple2<Object, Double> spatialObject2) {
        if (spatialObject1._2 > spatialObject2._2) {
            return 1;
        } else if (spatialObject1._2 < spatialObject2._2) {
            return -1;
...
public abstract class VisualizationOperator implements Serializable {
    ...
    private boolean spatialPartitioningWithoutDuplicates() throws Exception {
        this.distributedRasterColorMatrix =
                this.distributedRasterColorMatrix.mapToPair(
                        new PairFunction<Tuple2<Pixel, Integer>, Pixel, Integer>() {
                            @Override
                            public Tuple2<Pixel, Integer> call(
                                    Tuple2<Pixel, Integer> pixelDoubleTuple2) throws Exception {
                                Pixel newPixel =
                                        new Pixel(
                                                pixelDoubleTuple2._1().getX(),
                                                pixelDoubleTuple2._1().getY(),
                                                resolutionX,
                                                resolutionY);
                                ...
                                newPixel.setCurrentPartitionId(
                                        VisualizationPartitioner.CalculatePartitionId(
                                                ...
                                                partitionX,
                                                partitionY,
                                                (int) pixelDoubleTuple2._1.getX(),
                                                (int) pixelDoubleTuple2._1.getY()));
                                Tuple2<Pixel, Integer> newPixelDoubleTuple2 =
                                        new Tuple2<Pixel, Integer>(
                                                newPixel, pixelDoubleTuple2._2());
                                return newPixelDoubleTuple2;
                            }
                        });
        this.distributedRasterColorMatrix =
                this.distributedRasterColorMatrix.partitionBy(
                ...
                this.distributedRasterCountMatrix.flatMapToPair(
                        new PairFlatMapFunction<Tuple2<Pixel, Double>, Pixel, Double>() {
                            ...
                            public Iterator<Tuple2<Pixel, Double>> call(
                                    Tuple2<Pixel, Double> pixelDoubleTuple2) throws Exception {
                                VisualizationPartitioner vizPartitioner =
                                        new VisualizationPartitioner(
                                                resolutionX, resolutionY, partitionX, partitionY);
                                return vizPartitioner
                                        .assignPartitionIDs(pixelDoubleTuple2, photoFilterRadius)
                                        .iterator();
                            }
                        });
        this.distributedRasterCountMatrix =
                this.distributedRasterCountMatrix.partitionBy(
                        new VisualizationPartitioner(
                                this.resolutionX,
                                this.resolutionY,
                                this.partitionX,
            ...
            this.distributedRasterCountMatrix =
                    this.distributedRasterCountMatrix.mapPartitionsToPair(
                            new PairFlatMapFunction<
                                    Iterator<Tuple2<Pixel, Double>>, Pixel, Double>() {

                                @Override
                                public Iterator<Tuple2<Pixel, Double>> call(
                                        Iterator<Tuple2<Pixel, Double>> currentPartition)
                                        throws Exception {
                                    // This function uses an efficient algorithm to recompute the
                                    // pixel value. For each existing pixel,
                                    // we calculate its impact for all pixels within its impact
                                    // range and add its impact values.
                                    HashMap<Pixel, Double> pixelCountHashMap =
                                            new HashMap<Pixel, Double>();
                                    while (currentPartition.hasNext()) {
                                        Tuple2<Pixel, Double> currentPixelCount =
                                        ...
                                        if (centerPixelCoordinate._1() < 0
                                            ...
                                            // This center pixel is out of boundary so that we don't
                                            // record its sum. We don't plot this pixel on the final
                                            // sub image.
                                            continue;
                                        }
                                        // Find all pixels that are in the working pixel's impact
                                        // range
                                        for (int x = -photoFilterRadius;
                                                x <= photoFilterRadius;
                                                x++) {
                                            for (int y = -photoFilterRadius;
                                                    y <= photoFilterRadius;
                                                    y++) {
                                                int neighborPixelX = centerPixelCoordinate._1 + x;
                                                int neighborPixelY = centerPixelCoordinate._2 + y;
                                                if (currentPixelCount._1().getCurrentPartitionId()
                                                        < 0) {
                                                    throw new Exception(
                                                ...
                                                if (neighborPixelX < 0
                                                    ...
                                                    // This neighbor pixel is out of boundary so
                                                    // that we don't record its sum. We don't plot
                                                    // this pixel on the final sub image.
                                                    continue;
                                                }
                                                if (VisualizationPartitioner.CalculatePartitionId(
                                                    ...
                                                    // This neighbor pixel is not in this image
                                                    // partition so that we don't record its sum. We
                                                            ...
                                                            new Pixel(
                                                                    ...
                                                                    VisualizationPartitioner
                                                                            .CalculatePartitionId(
                                                                                    ...
                                                                                    neighborPixelX,
                                                                                    neighborPixelY));
                                                    pixelCountHashMap.put(
                                                            newPixel, neighborPixelCount);
                                                } else {
                                                    neighborPixelCount =
                                                            currentPixelCount._2()
                                                                    * PhotoFilterConvolutionMatrix[
                                    ...
                                    // Loop over the result map and convert the map. This is not
                                    // efficient and can be replaced by a better way.
                                    List<Tuple2<Pixel, Double>> resultSet =
                                            new ArrayList<Tuple2<Pixel, Double>>();
                                    Iterator<java.util.Map.Entry<Pixel, Double>> hashmapIterator =
                                            pixelCountHashMap.entrySet().iterator();
                                    while (hashmapIterator.hasNext()) {
                                        Map.Entry<Pixel, Double> cursorEntry =
                                                hashmapIterator.next();
                                        resultSet.add(
                                                new Tuple2<Pixel, Double>(
                                                        cursorEntry.getKey(),
                                                        cursorEntry.getValue()));
                                    }
            ...
            this.distributedRasterCountMatrix =
                    this.distributedRasterCountMatrix.flatMapToPair(
                            new PairFlatMapFunction<Tuple2<Pixel, Double>, Pixel, Double>() {
                                @Override
                                public Iterator<Tuple2<Pixel, Double>> call(
                                        Tuple2<Pixel, Double> pixelCount) throws Exception {
                                    Tuple2<Integer, Integer> centerPixelCoordinate =
                                            new Tuple2<Integer, Integer>(
                                                    (int) pixelCount._1().getX(),
                                                    (int) pixelCount._1().getY());
                                    List<Tuple2<Pixel, Double>> result =
                                            new ArrayList<Tuple2<Pixel, Double>>();
                                    for (int x = -photoFilterRadius; x <= photoFilterRadius; x++) {
                                        for (int y = -photoFilterRadius;
                                                y <= photoFilterRadius;
    ...
    protected boolean Colorize() {
        ...
        if (this.maxPixelCount < 0) {
            this.maxPixelCount =
                    this.distributedRasterCountMatrix.max(new RasterPixelCountComparator())._2;
        }
        ...
        this.distributedRasterColorMatrix =
                this.distributedRasterCountMatrix.mapValues(
                        new Function<Double, Integer>() {
                            ...
                            public Integer call(Double pixelCount) throws Exception {
                                ...
                                Double normalizedPixelCount =
                                        (currentPixelCount - minWeight)
                                                * 255
                                                / (maxWeight - minWeight);
        ...
        // logger.debug("[Sedona-VizViz][Colorize]output count
        // "+this.distributedRasterColorMatrix.count());
    ...
    protected boolean RenderImage(JavaSparkContext sparkContext) throws Exception {
        ...
        if (this.parallelRenderImage == true) {
                    ...
                    this.distributedRasterColorMatrix.mapPartitionsToPair(
                            new PairFlatMapFunction<
                                ...
                                public Iterator<Tuple2<Integer, ImageSerializableWrapper>> call(
                                    ...
                                    logger.info(
                                            "[Sedona-VizViz][Render]add a image partition into"
                                                    + " result set "
                    ...
                    this.distributedRasterImage.reduceByKey(
                            new Function2<
                                ...
                                public ImageSerializableWrapper call(
                                        ImageSerializableWrapper image1,
                                        ImageSerializableWrapper image2)
                                        throws Exception {
            ...
            // logger.debug("[Sedona-VizViz][RenderImage]Merged all images into one image. Result
            // size should be 1. Actual size is "+this.distributedRasterImage.count());
    ...
    */
    ...
    protected Color EncodeToColor(int normailizedCount) throws Exception {
        if (controlColorChannel.equals(Color.RED)) {
            ...
            throw new Exception(
                    "[Sedona-VizViz][GenerateColor] Unsupported changing color color type. It"
                            + " should be in R,G,B");
        }
     ...
     * @return the java pair RDD
     */
    protected JavaPairRDD<Pixel, Double> Rasterize(
        ...
        JavaPairRDD<Pixel, Double> spatialRDDwithPixelId =
                rawSpatialRDD.flatMapToPair(
                        new PairFlatMapFunction<Object, Pixel, Double>() {
                            @Override
                            public Iterator<Tuple2<Pixel, Double>> call(Object spatialObject)
                                    throws Exception {

                                if (spatialObject instanceof Point) {
                                    return RasterizationUtils.FindPixelCoordinates(
                                                    resolutionX,
                                                    resolutionY,
                                                    datasetBoundary,
                                                    (Point) spatialObject,
                                    ...
                                    throw new Exception(
                                            "[Sedona-VizViz][Rasterize] Unsupported spatial object"
                                                    + " types. Sedona-VizViz only supports Point,"
                                                    + " Polygon, LineString");
                                }
                            }
                        });
        // JavaPairRDD<Integer, Double> originalImage =
        // sparkContext.parallelizePairs(this.countMatrix);
        // spatialRDDwithPixelId = spatialRDDwithPixelId.union(originalImage);
        spatialRDDwithPixelId =
                spatialRDDwithPixelId.filter(
                        new Function<Tuple2<Pixel, Double>, Boolean>() {
                            @Override
                            public Boolean call(Tuple2<Pixel, Double> pixelCount) throws Exception {
                                return !(pixelCount._1().getX() < 0)
                                        && !(pixelCount._1().getX() > resolutionX)
                                        && !(pixelCount._1().getY() < 0)
                                        && !(pixelCount._1().getY() > resolutionY);
                            }
                        });
        ...
        */
        ...
        // logger.debug("[Sedona-VizViz][Rasterize]output count
        // "+this.distributedRasterCountMatrix.count());
    ...
    protected JavaPairRDD<Pixel, Double> Rasterize(
            JavaSparkContext sparkContext,
            JavaPairRDD<Polygon, Long> spatialPairRDD,
            boolean useSparkDefaultPartition) {
        logger.info("[Sedona-VizViz][Rasterize][Start]");
        JavaPairRDD<Pixel, Double> spatialRDDwithPixelId =
                spatialPairRDD.flatMapToPair(
                        new PairFlatMapFunction<Tuple2<Polygon, Long>, Pixel, Double>() {
                            @Override
                            public Iterator<Tuple2<Pixel, Double>> call(
                                    Tuple2<Polygon, Long> spatialObject) throws Exception {
                                return RasterizationUtils.FindPixelCoordinates(
                                                resolutionX,
                                                resolutionY,
                                                datasetBoundary,
                                                spatialObject._1(),
                                                reverseSpatialCoordinate,
                                                new Double(spatialObject._2()))
                                        .iterator();
                            }
                        });
        // JavaPairRDD<Pixel, Double> originalImage =
        // sparkContext.parallelizePairs(this.countMatrix);
        // JavaPairRDD<Integer,Double> completeSpatialRDDwithPixelId =
        // spatialRDDwithPixelId.union(originalImage);
