...
public class VisualizationPartitioner extends Partitioner implements Serializable {
     ...
     */
    public VisualizationPartitioner(
            int resolutionX, int resolutionY, int partitionX, int partitionY) throws Exception {
        ...
        if (this.resolutionX % partitionX != 0 || this.resolutionY % partitionY != 0) {
            throw new Exception(
                    "[VisualizationPartitioner][Constructor] The given partition number fails to"
                            + " exactly divide the corresponding resolution axis.");
        }
    ...
    public static int CalculatePartitionId(
        ...
        Tuple2<Integer, Integer> partitionId2d =
                Calculate2DPartitionId(
                        resolutionX, resolutionY, partitionX, partitionY, coordinateX, coordinateY);
        int partitionId = -1;
        try {
            partitionId =
                    RasterizationUtils.Encode2DTo1DId(
                            partitionX, partitionY, partitionId2d._1, partitionId2d._2);
        } catch (Exception e) {
     ...
     * @return
     */
    public static Tuple2<Integer, Integer> Calculate2DPartitionId(
            int resolutionX,
            int resolutionY,
            int partitionX,
            int partitionY,
     ...
     * @return the list
     */
    public List<Tuple2<Pixel, Double>> assignPartitionIDs(
            Tuple2<Pixel, Double> pixelDoubleTuple2, int photoFilterRadius) {
        ...
        // Tuple2<Integer,Integer> pixelCoordinateInPartition = new Tuple2<Integer,
        // Integer>(pixelDoubleTuple2._1().getX()%partitionIntervalX,pixelDoubleTuple2._1().getY()%partitionIntervalY);

        int[] boundaryCondition = {-1, 0, 1};
        for (int x : boundaryCondition) {
            for (int y : boundaryCondition) {
                int duplicatePartitionId =
                        CalculatePartitionId(
                                resolutionX,
                                resolutionY,
                                partitionX,
                                partitionY,
                                (int) pixelDoubleTuple2._1().getX() + x * photoFilterRadius,
                                (int) pixelDoubleTuple2._1().getY() + y * photoFilterRadius);
                if (duplicatePartitionId != partitionId && duplicatePartitionId >= 0) {
                    ...
                    duplicatePixelList.add(
                            new Tuple2<Pixel, Double>(newPixelDuplicate, pixelDoubleTuple2._2()));
                }
            }
        }
