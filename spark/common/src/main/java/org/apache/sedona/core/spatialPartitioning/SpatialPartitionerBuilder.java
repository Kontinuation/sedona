...
public class SpatialPartitionerBuilder {
     ...
     */
    public SpatialPartitionerBuilder(GridType gridType, int numPartitions, int totalSamples, Envelope boundary) {
        ...
        switch (gridType) {
            case EQUALGRID: {
                ...
                QuadtreePartitioning quadtreePartitioning = new QuadtreePartitioning(totalSamples, boundary,
            ...
            case KDBTREE: {
                // According to the research efforts by W. Wright, the average utilization of B-tree page is
                // ln(2), which is approximately 70%. So this is a more accurate estimation of maxItemsPerNode:
                //
                // int maxItemsPerNode = Math.max((int)(totalSamples / (numPartitions * 0.70)), 1);
                //
                // We are not using this formula here because having slightly more spatial partitions usually yields
                // better performance.
    ...
    /**
     * Add samples to the spatial partitioner, so that the partitioner can generate spatial partitions according
    ...
    public static SpatialPartitioner buildSpatialPartitionerForSpatialJoin(
            ...
            GridType gridType, Envelope bound, int numPartitions,
            List<Envelope> samples, long estimatedLeftCount, List<Envelope> otherSamples, long estimatedRightCount) {
        switch (strategy) {
            case NAIVE:
                return buildSpatialPartitionerForSpatialJoinNaive(gridType, bound, numPartitions,
                        samples, estimatedLeftCount, otherSamples, estimatedRightCount);
            case SUBSAMPLING:
                return buildSpatialPartitionerForSpatialJoinSubSampling(gridType, bound, numPartitions,
                        samples, estimatedLeftCount, otherSamples, estimatedRightCount);
            default:
                throw new IllegalArgumentException("Unsupported spatial partition building strategy " + strategy);
        }
    }
    ...
    public static SpatialPartitioner buildSpatialPartitionerForSpatialJoinNaive(
        ...
        SpatialPartitionerBuilder builder = new SpatialPartitionerBuilder(gridType, numPartitions,
                totalSamplesInBounds, bound);
    ...
    public static SpatialPartitioner buildSpatialPartitionerForSpatialJoinSubSampling(
            GridType gridType, Envelope bound, int numPartitions,
            List<Envelope> left, long estimatedLeftCount, List<Envelope> right, long estimatedRightCount) {
        // Both left and right are in-bound samples. We need the total count of both datasets to do a further sub-sampling
        ...
        return buildSpatialPartitionerForSpatialJoinNaive(gridType, bound, numPartitions,
                leftSubSamples, estimatedLeftCount, rightSubSamples, estimatedRightCount);
    }
