...
class SerdeAwareFunctionSpec extends TestBaseScala {
  ...
  describe("SerdeAwareFunction") {
    it("should save us some serialization and deserialization cost for geometries") {
      ...
      mocked
        .when(() => GeometrySerializer.deserialize(any(classOf[Array[Byte]])))
        .thenReturn(stubGeom)
      mocked
        .when(() => GeometrySerializer.serialize(any(classOf[Geometry])))
        .thenReturn(Array[Byte](1, 2, 3))
      ...
      try {
        ...
        mocked.verify(() => GeometrySerializer.deserialize(any(classOf[Array[Byte]])), atMost(0))
        mocked.verify(() => GeometrySerializer.serialize(any(classOf[Geometry])), atMost(1))
      } finally {
        // Undo the mock
        mocked.close()
      }
    ...
    it("should save us some serialization and deserialization cost for rasters") {
      ...
      mocked
        .when(() => Serde.deserialize(any(classOf[Array[Byte]]), any(classOf[Array[Byte]])))
        .thenReturn(fromArcInfoAsciiGrid(ascGrid.getBytes))
      mocked
        .when(() => Serde.serialize(any(classOf[GridCoverage2D]), any(classOf[Boolean])))
        .thenReturn(Array[Byte](1, 2, 3))

      val expr = RS_NumBands(Seq(RS_FromArcInfoAsciiGrid(Seq(Literal(ascGrid.getBytes)))))
