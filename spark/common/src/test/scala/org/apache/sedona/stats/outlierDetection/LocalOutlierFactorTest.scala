...
class LocalOutlierFactorTest extends TestBaseScala {
  ...
  describe("LocalOutlierFactor") {
    it("returns correct results") {
      val initialMetricCount = MetricsRegistrator.getOrCreate.LOFPerform.getCount
      val exactResultDf = LocalOutlierFactor.localOutlierFactor(get_data(), 4)
      assert(exactResultDf.count() == 11)
      assert(
        exactResultDf
          .filter(f.abs(f.col("expected_lof") - f.col("lof")) < .00000001)
          .count() == 11)

      // Testing shows in this case approx is giving exact results
      val approxResultDf =
        LocalOutlierFactor.localOutlierFactor(get_data(), 4, approximateKNN = true)
      assert(approxResultDf.count() == 11)
      assert(
        approxResultDf
          .filter(f.abs(f.col("expected_lof") - f.col("lof")) < .00000001)
          .count() == 11)
      ...
      assert((MetricsRegistrator.getOrCreate.LOFPerform.getCount - initialMetricCount) == 2)
    }
  }
}
