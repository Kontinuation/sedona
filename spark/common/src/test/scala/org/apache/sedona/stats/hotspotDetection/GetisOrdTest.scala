...
import org.apache.sedona.stats.{MetricsRegistrator, Weighting}
...
class GetisOrdTest extends TestBaseScala {
  ...
  describe("glocal") {
    it("returns one row per input row with expected columns, binary") {
      val initialMetricCount = MetricsRegistrator.getOrCreate.GLocalPerform.getCount
      ...
      assert((MetricsRegistrator.getOrCreate.GLocalPerform.getCount - initialMetricCount) == 1)
    }
