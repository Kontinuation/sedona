...
class geoparquetIOTests extends TestBaseScala with BeforeAndAfterAll {
  ...
  describe("stac-geoparquet as an alias of geoparquet") {
    it("stac-geoparquet should work as geoparquet") {
  ...
  it("should support nested schema pruning") {
    val df = sparkSession.read
      .format("geoparquet")
      .load(overtureBBOX)
      .select("id", "bbox.xmin")
    val scans = df.queryExecution.executedPlan.collect { case p: FileSourceScanLike =>
      p
    }
    assert(scans.nonEmpty)
    scans.foreach { scan =>
      val fields = scan.requiredSchema.fields
      assert(fields.length == 2)
      // bbox nested column is pruned to select only one field
      assert(fields(1).dataType.isInstanceOf[StructType])
      assert(fields(1).dataType.asInstanceOf[StructType].length == 1)
    }
  }
