...
class StacDataSourceTest extends TestBaseScala {
  ...
  it("select SQL with spatial filter with remote service endpoint") {
    val dfStac = sparkSession.read.format("stac").load(STAC_COLLECTION_SERVICE)
    dfStac.createOrReplaceTempView("STACTBL")
    ...
    val dfSelect = sparkSession.sql(
      "SELECT id, geometry " +
        "FROM STACTBL " +
        "WHERE st_intersects(ST_GeomFromText('POLYGON((9.68 47.94, 9.84 47.94, 9.84 48.92, 9.68 48.92, 9.68 47.94))'), geometry)")

    val physicalPlan = dfSelect.queryExecution.executedPlan.toString()
    assert(physicalPlan.contains(
      "PushedSpatialFilters -> LeafFilter(geometry,INTERSECTS,POLYGON ((9.68 47.94, 9.84 47.94, 9.84 48.92, 9.68 48.92, 9.68 47.94)))"))
        ...
        MapType(
          ...
          StructType(
            Seq(
              ...
              StructField("rast", new RasterUDT(), nullable = true))),
