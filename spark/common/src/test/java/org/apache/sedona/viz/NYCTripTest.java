...
class NYCTripMapper implements FlatMapFunction<String, Object> {
    ...
    public Iterator call(String line) throws Exception {
        ...
        coordinatesList.add(
                new Coordinate(
                        Double.parseDouble(lineSplitList.get(5)) * -1.0,
                        Double.parseDouble(lineSplitList.get(6))));
        coordinatesList.add(
                new Coordinate(
                        Double.parseDouble(lineSplitList.get(8)) * -1.0,
                        Double.parseDouble(lineSplitList.get(9))));
        spatialObject =
...
public class NYCTripTest {
    ...
    public static void setUpBeforeClass() throws Exception {
        ...
        prop = new Properties();
        inputProp =
                ScatterplotTest.class.getClassLoader().getResourceAsStream("point.test.properties");
        prop.load(inputProp);
        // PointInputLocation =
        ...
        // "file://"+ScatterplotTest.class.getClassLoader().getResource("county.csv").getPath();
        ...
        inputProp =
                ScatterplotTest.class
                        .getClassLoader()
                        .getResourceAsStream("linestring.test.properties");
        prop.load(inputProp);
        // LineStringInputLocation =
        // "file://"+ScatterplotTest.class.getClassLoader().getResource("trip-sample.csv").getPath();
    ...
    public void testLineStringRDDVisualization() throws Exception {
        ...
        PointRDD spatialRDD =
                new PointRDD(
                        sparkContext, PointInputLocation, PointNumPartitions, nycTripPointMapper);
        HeatMap visualizationOperator =
                new HeatMap(resolutionX, resolutionY, NYCBoundary, false, 5);
        visualizationOperator.Visualize(sparkContext, spatialRDD);

        ImageGenerator imageGenerator = new ImageGenerator();
        imageGenerator.SaveRasterImageAsLocalFile(
                visualizationOperator.rasterImage, "./target/heatmap/NYCTrip", ImageType.PNG);
    }
}
