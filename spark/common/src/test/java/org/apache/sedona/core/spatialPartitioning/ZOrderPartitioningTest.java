...
public class ZOrderPartitioningTest extends TestCase {
  ...
  public void testCalculateZOrder() {
    ZOrderPartitioning zOrderPartitioning =
        new ZOrderPartitioning(new Envelope(-20, 30, -100, 100), 4);
  ...
  /**
   * Test the case where the number of partitions is less than the number of samples. The number of
   * ranges should match the number of partitions.
   */
  @Test
  public void testCreateZOrderRangesLessPartitions() {
    ZOrderPartitioning zOrderPartitioning =
        new ZOrderPartitioning(new Envelope(-20, 30, -100, 100), 2);
  ...
  /**
   * Test the case where the number of partitions is more than the number of samples. The number of
   * ranges should match the number of samples.
   */
  @Test
  public void testCreateZOrderRangesLessSamples() {
    ...
    List<Envelope> samples = Arrays.asList(new Envelope(10, 20, 10, 20));
    List<Range<Long>> ranges = zOrderPartitioning.createZOrderRanges(samples, 0);
    assertEquals("Number of ranges should match the requested number.", 1, ranges.size());
  }

  /**
   * Test the case where samples are empty. The number of ranges should be 1 (including the whole
   * boundary).
   */
  @Test
  ...
  /**
   * Test the case where partition is set to 0. The method should throw an IllegalArgumentException.
   */
  @Test
  public void testCreateZOrderRangesInvalidPartitions() {
    ZOrderPartitioning zOrderPartitioning =
        new ZOrderPartitioning(new Envelope(-20, 30, -100, 100), 0);
  ...
  public void testCreateZOrderRangesEqualPartitions() {
    ...
    List<SampleWithZOrder<Envelope>> orderredSamples =
        samples.stream()
            .map(
                sample ->
                    new SampleWithZOrder<>(sample, zOrderPartitioning.calculateZOrder(sample)))
            .sorted((a, b) -> Long.compare(a.getZOrder(), b.getZOrder()))
            .collect(Collectors.toList());

    assertEquals(
        "Number of ordered sample should match the requested number.",
        samples.size(),
        orderredSamples.size());
    ...
    Iterator<Tuple2<Integer, Geometry>> iterator1 =
        tree.placeObject(factory.createPoint(new Coordinate(3, 3)));
    ...
    Iterator<Tuple2<Integer, Geometry>> iterator2 =
        nonOverlappedTree.placeObject(factory.createPoint(new Coordinate(3.3, 4.4)));
  ...
  public void testCreateZOrderRangesWithHotspots() {
    ZOrderPartitioning zOrderPartitioning =
        new ZOrderPartitioning(new Envelope(-20, 30, -100, 100), 8);
