...
public class AdaptiveIndexLookupJudgementTest {
    ...
    public void testEmptyDatasets() {
        ...
        // Left side is empty
        resultIterator =
                judgement.call(0, Collections.emptyIterator(),
                        new SingletonIterator(factory.createPoint(new Coordinate(0, 0))));
        assertFalse(resultIterator.hasNext());

        // Right side is empty
        resultIterator =
                judgement.call(0, new SingletonIterator(factory.createPoint(new Coordinate(0, 0))),
    ...
    private void testExecutionMode(ExecutionMode executionMode) {
        ...
        IndexType[] indexTypes = new IndexType[]{
                IndexType.RTREE,
                IndexType.QUADTREE
        };
        IndexBuildSide[] indexBuildSides = new IndexBuildSide[]{
                IndexBuildSide.LEFT,
                IndexBuildSide.RIGHT
        };
    ...
    private static void verifyResult(Iterator<Pair<Geometry, Geometry>> resultIterator,
            List<Geometry> datasetA, List<Geometry> datasetB, SpatialPredicate predicate) {
        Set<Pair<Geometry, Geometry>> expected = new HashSet<>();
        SpatialPredicateEvaluators.SpatialPredicateEvaluator evaluator = SpatialPredicateEvaluators.create(predicate);
