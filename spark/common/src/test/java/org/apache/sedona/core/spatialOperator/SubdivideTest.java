...
public class SubdivideTest extends TestBase {
    ...
    public void subdivideSpatialRDD() {
        ...
        Subdivide.SubdivideRDDOptions subdivideRDDOptions = new Subdivide.SubdivideRDDOptions(options, true, true);
        ...
        assertEquals(geoms.size(), resultRDD.rawSpatialRDD.map(geom -> {
            Subdivide.SubdividedPart part = (Subdivide.SubdividedPart) geom.getUserData();
            return part.id;
        }).distinct().count());

        // Subdivide correctness check
        List<Iterable<Geometry>> subdividedGeoms = resultRDD.rawSpatialRDD.groupBy(geom -> {
            Subdivide.SubdividedPart part = (Subdivide.SubdividedPart) geom.getUserData();
            return part.id;
        }).values().collect();
    ...
    public void attachId() {
        ...
        List<String> resultData = resultRdd.map(Tuple2::_2).collect().stream().sorted().collect(Collectors.toList());
    ...
    public void noOpSubdivideSpatialRDD() {
        ...
        JavaPairRDD<Long, Geometry> keyedResultRDD = resultRDD.getRawSpatialRDD().keyBy((geom) -> {
            Subdivide.SubdividedPart part = (Subdivide.SubdividedPart) geom.getUserData();
            return part.id;
        });
