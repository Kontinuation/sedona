...
class STH3Functions extends TestBaseScala with Matchers with GeometrySample with GivenWhenThen {
  ...
  describe("should pass ST_H3CellIDs"){
    ...
    it("use ST_H3CellIDs for spatial join") {
      ...
      require(
        res.get(0).asInstanceOf[Long] == 5
      )
      require(
        res.get(0).asInstanceOf[Long] <= res.get(1).asInstanceOf[Long]
      )
    }
    ...
    it("should correctly calculate the distance between two h3 cells") {
      Given("DataFrame with valid Geometries")
      val geometryTable = Seq(
        Seq("POINT(1 2)", "POINT(1.23 1.59)")
      ).map(geom => Tuple2(wktReader.read(geom(0)), wktReader.read(geom(1)))).toDF("lgeom", "rgeom")
      ...
      val distance = geometryTable
        ...
        .select(expr("ST_H3CellDistance(lcell, rcell)")).collectAsList().get(0).get(0)
    ...
    it("should correctly retrieve the KRing neighbors of given cellId") {
      ...
      val exacts = geometryTable.select(
        expr("ST_H3KRing(ST_H3CellIDs(geom, 8, true)[0], 3, true)")
      ).collectAsList().get(0).get(0).asInstanceOf[mutable.WrappedArray[Long]].toArray
      val expects = Functions.h3KRing(
        H3Utils.coordinateToCell(new Coordinate(1, 2), 8),
        3,
        true
      )
