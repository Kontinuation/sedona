...
from sedona.stats.clustering.dbscan import dbscan, get_knee_locator
...
class TestDBScan(TestBase):
    ...
    def test_get_knee_locator(self):
        df = self.create_sample_dataframe()
        knee_locator = get_knee_locator(df, 2, interp_method="polynomial", polynomial_degree=9)

        assert isinstance(knee_locator, KneeLocator)
        assert knee_locator.curve == "convex"
        assert knee_locator.polynomial_degree == 9
