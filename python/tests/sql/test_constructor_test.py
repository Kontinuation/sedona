...
class TestConstructors(TestBase):
    ...
    def test_st_makepointm(self):
        ...
        point_df = self.spark.sql(
            "SELECT ST_AsText(ST_MakePointM(1.2345, 2.3456, 3.4567))"
        )
    ...
    def test_st_geom_from_ewkt(self):
        input_df = self.spark.createDataFrame(
            [("SRID=4269;LineString(1 2, 3 4)",)], ["ewkt"]
        )
    ...
    def test_st_geom_from_wkt_3d(self):
        ...
        polygon_df = self.spark.sql(
            "select ST_GeomFromWkt(wkt) as geomn from input_wkt"
        )
    ...
    def test_st_geometry_from_text(self):
        ...
        polygon_wkt_df.createOrReplaceTempView("polygontable")
        polygon_df = self.spark.sql(
            "select ST_GeometryFromText(polygontable._c0) as countyshape from polygontable"
        )
        assert polygon_df.count() == 100

        polygon_df = self.spark.sql(
            "select ST_GeomFromText(polygontable._c0, 4326) as countyshape from polygontable"
        )
        assert polygon_df.count() == 100
    ...
    def test_st_linestring_from_wkb(self):
        linestring_ba = self.spark.sql(
            "select unhex('0102000000020000000000000084d600c00000000080b5d6bf00000060e1eff7bf00000080075de5bf') as wkb"
        )
        actual = linestring_ba.selectExpr("ST_AsText(ST_LineStringFromWKB(wkb))").take(
            1
        )[0][0]
        expected = "LINESTRING (-2.1047439575195312 -0.354827880859375, -1.49606454372406 -0.6676061153411865)"
        assert actual == expected

        linestring_s = self.spark.sql(
            "select '0102000000020000000000000084d600c00000000080b5d6bf00000060e1eff7bf00000080075de5bf' as wkb"
        )
        actual = linestring_s.selectExpr("ST_AsText(ST_LinestringFromWKB(wkb))").take(
            1
        )[0][0]
        assert actual == expected
    ...
    def test_Mline_from_text(self):
        input_df = self.spark.createDataFrame(
            [("MULTILINESTRING((1 2, 3 4), (4 5, 6 7))",)], ["wkt"]
        )
        input_df.createOrReplaceTempView("input_wkt")
    ...
    def test_mpoint_from_text(self):
        baseDf = self.spark.sql(
            "SELECT 'MULTIPOINT ((10 10), (20 20), (30 30))' as geom, 4326 as srid"
        )
