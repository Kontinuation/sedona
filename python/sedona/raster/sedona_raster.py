...
class OutDbSedonaRaster(OutDbSedonaRasterBase):
    ...
    def _as_rasterio(
        self, load_bands: Optional[List[int]]
    ) -> Tuple[DatasetReader, Optional[np.ndarray]]:
        if self.rasterio_dataset_reader is not None:
        ...
        if not self.is_vrt_needed(src):
            ...
            ds, arr = _rasterio_open_memfile(
                src_path, self.rasterio_memfile, driver="VRT", band_indices=load_bands
            )
    ...
    def is_vrt_needed(self, ds: DatasetReader) -> bool:
        return not (
            ...
            and self._height == ds.height
            and self._outdb_meta.band_indices == [k - 1 for k in ds.indexes]
        )
...
class LazyLoadOutDbSedonaRaster(OutDbSedonaRasterBase):
    ...
    def __init__(self, path: str, params: Optional[Dict[str, str]]):
        super().__init__(
            -1, -1, [], AffineTransform(0, 0, 0, 0, 0, 0, PixelAnchor.UPPER_LEFT), ""
        )
    ...
    def _ensure_loaded(self):
        ...
        for idx, band_idx in enumerate(ds.indexes):
            sample_dimension = SampleDimension(
                f"band_{band_idx}", 0.0, 1.0, nodatavals[idx]
            )
    ...
    def set_path(self, path: str):
        if self.rasterio_dataset_reader is not None:
            raise RuntimeError(
                "Cannot set the path of an already loaded lazy-loaded out-db raster"
            )
        self._path = path
