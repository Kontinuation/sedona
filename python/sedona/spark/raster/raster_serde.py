...
def _deserialize(bio: BytesIO, raster_type: int) -> SedonaRaster:
    ...
    if raster_type != RasterTypes.LAZY_LOAD_OUT_DB:
        ...
        if raster_type == RasterTypes.IN_DB:
            ...
            return OutDbSedonaRaster(
                width, height, bands_meta, affine_trans, crs_wkt, outdb_meta
            )
    else:
...
def _read_outdb_meta(bio: BytesIO) -> OutDbMeta:
    (data_type,) = struct.unpack("=i", bio.read(4))
    band_indices = _read_int_array(bio)
    path = _read_utf8_string(bio)
    ...
    params = _read_utf8_string_map(bio)
    return OutDbMeta(data_type, band_indices, path, params)
